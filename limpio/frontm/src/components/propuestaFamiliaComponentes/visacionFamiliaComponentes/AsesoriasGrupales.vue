    ...
    <v-dialog v-model="asesoria_grupal_dialog">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-spacer></v-spacer>
          <v-btn @click="saveAsesoriaGrupal()">Guardar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  props: ["asesorias_grupales", "convocatoria"],
  ...
  methods: {
    ...
    openConvDialog(){
      let estado_id = this.asesoria_grupal_edited.bit_estado_id ;
      if (estado_id == 21 || estado_id == null) this.openDialogDesestimar()
    ...
    changeStatusAsesorias() {
      var status = this.asesoria_grupal_edited.bit_estado_id
      ...
      if (this.asesoria_grupal_edited.comentario == null){
        ...
        changeStatusAsesoriasFromApi(status, solucion_id, this.asesoria_grupal_edited.comentario)
        .then((response) => {
    ...
    openAsesoriaGrupalDialog(item) {
      getAsesoriasGrupalesOptionsFromApi(this.convocatoria.id)
        .then((res) => {
          ...
          var bit_estado_actual_id = this.asesoria_grupal_edited.bit_estado_actual_id
          console.log(this.asesoria_grupal_edited);
          if (bit_estado_actual_id==null){
            getStatusEntidad(20)
              .then((response) => {
                if (response.data.type == "success") {
                  this.asesorias_estados_options = response.data.data
                  this.asesoria_grupal_edited.bit_estado_actual_id = 20
                  this.asesoria_grupal_edited.bit_estado_actual_id = parseInt(this.asesoria_grupal_edited.bit_estado_actual_id)
                } else {
                  this.showMessage(response.data.message, "warning");
                }
              })
