    ...
    <v-dialog v-model="solucion_dialog" width="98%">
      <v-card>
        ...
        <v-card-text>
          <v-form>
            <v-select
              v-show="this.solucion_edited.propuesta_solucion_id !== null"
...
export default {
  ...
  methods: {
    ...
    openConvDialog(){
      ...
      console.log(this.estados_soluciones[1].value)
      if (estado_id != this.estados_soluciones[1].value) this.convDialog = true
    ...
    getPropuestaPic() {
      getPropuestaPicFromApi(this.beneficiario_id).then((response) => {
        ...
        this.estados_soluciones = response.data.data.estados_soluciones;
