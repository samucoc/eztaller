...
$xajax->setRequestURI("sg_informe_asignacion_cupo.php");
...
function Grabar($data){
	...
	if ($persona != ''){
            $and .= " and rut_pers = '".$persona."' " ;
        }
        
        $sql_ab = "select distinct rut_pers,	cupo,	fecha,	usuario,	mes,	anio
                    from persona_tiene_cupos 
                    where
                        mes between '".$mes_1."' and '".$mes_2."' 
			and anio between '".$anio_1."' and '".$anio_2."'
			and rut_pers <> ''
			$and
	...
	if (mysql_num_rows($res_ab) > 0){
		...
		$i = 1;
                while ($line_ab = mysql_fetch_array($res_ab)){
                    ...
                    $sql_pr = "select concat(pers_nombre,' ',pers_ape_pat) as pers_nombre_com from personas where pers_rut = '".$line_ab[0]."'";
                    ...
                    $nombre_persona = @mysql_result($res_pr,0,"pers_nombre_com");
						...
						array_push($arrRegistros, array("item"		=>	$i,
										...
										"monto"     	=> 	$line_ab['cupo'],
										"fecha"         => 	$line_ab['mes'],
										"usuario"       => 	$line_ab['anio'],
										"temp"       => 	$line_ab['usuario']));
						$i++;
                    } 
		$_SESSION["alycar_matriz"] = $arrRegistros;
                $miSmarty->assign('arrRegistros', $arrRegistros);
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_asignacion_cupo_list.tpl'));
...
$miSmarty->display('sg_informe_asignacion_cupo.tpl');
