...
$xajax->setRequestURI("sg_informe_gratificaciones.php");
...
function Grabar($data){
	...
	if (($OBLItxtFecha1 != '')){
		...
		$and .= " and gratificaciones.fecha between '".$OBLItxtFecha1."' and '".$OBLItxtFecha2."' ";
	}

	if (($centro_costo != '')&&($centro_costo != '- - Seleccione - -')){
		$and .= " and gratificaciones.centro_costo = '".$centro_costo."' ";
	}
	
	if (($grupo != '')){
		$and .= " and gratificaciones.grupo = '".$grupo."' ";
	}
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_array($res_pd)) {
			...
			array_push($arrRegistros, array("fecha"  => 	$line_pd['fecha'],
							"rut_trab"	=> 	$line_pd['trabajador'].'-'.dv($line_pd['trabajador']),
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_gratificaciones_list.tpl'));
...
function CargaPagina($data){
	...
	$objResponse->addScript("xajax_CargaSelect(xajax.getFormValues('Form1'),'cboTipo','tipo_gratificaciones','','Todos','ta_ncorr','nombre', '')");
...
$miSmarty->display('sg_informe_gratificaciones.tpl');
