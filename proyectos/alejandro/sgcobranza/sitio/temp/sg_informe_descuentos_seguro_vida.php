...
$xajax->setRequestURI("sg_informe_descuentos_seguro_vida.php");
...
function Grabar($data){
	...
	if (($OBLItxtFecha1 != '') && ($OBLItxtFecha2 != '') ){
		...
		$and .= " and seguro_vida.fecha between '".$OBLItxtFecha1."' and '".$OBLItxtFecha2."' ";
	}
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		$arrRegistros = ordenar_matriz_multidimensionada($arrRegistros,'trabajador','ASC');
		$_SESSION["alycar_matriz"] 				= 	$arrRegistros;
		
		$miSmarty->assign('arrRegistros', $arrRegistros);
		$miSmarty->assign('total', $total);		
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_descuentos_seguro_vida_list.tpl'));
...
$miSmarty->display('sg_informe_descuentos_seguro_vida.tpl');
