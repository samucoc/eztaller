...
$xajax->setRequestURI("sg_anticipo_ingresar.php");
...
function Grabar($data){
	...
	//if (($mes3 == $mes2)&&($anio3 == $anio2)&&($dia3 <= $dia2)) {
		//echo $fecha.$monto.$trabajador; 
		if (($fecha!='')&&($monto!='')&&($trabajador!='')){
			list($dia3,$mes3,$anio3) = explode('/', $fecha);$fecha = $anio3."-".$mes3."-".$dia3;
			$fecha_ant = "";
			//echo $_SESSION['anticipo_grupo'];
			if (!isset($_SESSION['anticipo_grupo'])){
				...
				$grupo 		= $_SESSION['anticipo_grupo'] ;
				$sql_grupo = "select grupo, fecha, estado
								from anticipos
								where grupo = '".$_SESSION['anticipo_grupo']."'";
				$res_grupo = mysql_query($sql_grupo,$conexion);
				if (mysql_num_rows($res_grupo)==0){
					$grupo = $_SESSION['anticipo_grupo'];
					$estado = 'PAGADO';
			...
			if ($fecha_ant==$fecha){
					$sql = "insert into anticipos ( grupo, sector, `centro_costo`, `fecha`, 
									`trabajador`, 
									`tipo`, `monto`, `caja`, `observacion`, 
									`estado`, `usuario`, `fecha_digitacion`, 
									 tipo_trab) 
							values ('$grupo','$sector','$centro_costo','$fecha','$trabajador','$tipo','$monto','$caja','$observacion','PAGADO','$usuario','$fecha_digitacion', '$tt')";
					$res = mysql_query($sql,$conexion) or die(mysql_error());
				}
			$sql_buscar = "select anticipo_ncorr, fecha, trabajador, monto, caja, observacion
							from anticipos 
							where grupo = '".$_SESSION['anticipo_grupo']."'";
			...
			while ($row=mysql_fetch_array($res_buscar)){
				...
				array_push($arrRegistros, array(
								...
								"fecha"			=> $row['fecha'],
								"codigo"		=> $row['anticipo_ncorr']
								));
				$total = $total + $cantidad;
				}
			$miSmarty->assign('arrRegistros', $arrRegistros);
			$miSmarty->assign('total', $total);
			$miSmarty->assign('grupo', $_SESSION['anticipo_grupo']);
			$objResponse->addAssign("divresultado", "innerHTML", $miSmarty->fetch('sg_anticipo_ingresar_list.tpl'));	
...
function CargaPagina($data){
	...
	$objResponse->addAssign("grupo", "innerHTML", $_SESSION['anticipo_grupo'] );
  	return $objResponse->getXML();
...
function Eliminar($data, $ncorr){
	...
	$sql_4 = "select grupo from sgvales.anticipos where anticipo_ncorr = '".$ncorr."'";
	$res_4 = mysql_query($sql_4,$conexion) or die(mysql_error());
	$row_4 = mysql_fetch_array($res_4);
	$grupo = $row_4['grupo'];
	
	$sql_5 = "delete from sgvales.anticipos where anticipo_ncorr = '".$ncorr."'";
...
$miSmarty->display('sg_anticipo_ingresar.tpl');
