...
$xajax->setRequestURI("sg_gratificacion_ingresar.php");
...
function Grabar($data){
	...
	if (($mes3 == $mes2)&&($anio3 == $anio2)&&($dia3 <= $dia2)) {
		if (($fecha!='')&&($monto!='')&&($trabajador!='')){
			...
			$fecha_ant = "";
			if (!isset($_SESSION['gratificacion_grupo'])){
				...
				$grupo 		= $_SESSION['gratificacion_grupo'] ;
				$sql_grupo = "select grupo, fecha, estado
								from gratificaciones
								where grupo = '".$_SESSION['gratificacion_grupo']."'";
				$res_grupo = mysql_query($sql_grupo,$conexion);
				if (mysql_num_rows($res_grupo)==0){
					$grupo = $_SESSION['gratificacion_grupo'];
					$estado = 'PAGADO';
			...
			$_SESSION['gratificacion_grupo'] = $grupo ;
			...
			if ($fecha_ant==$fecha){
					$sql = "insert into gratificaciones ( grupo, `centro_costo`, `fecha`, `trabajador`, `caja`, `monto`, `observacion`, `estado`, `usuario`, `fecha_digitacion`) 
							values ('$grupo','$centro_costo','$fecha','$trabajador','$caja','$monto','$observacion','ESPERANDO_CORREO','$usuario','$fecha_digitacion')";
					$res = mysql_query($sql,$conexion) or die(mysql_error());
				}
			$sql_buscar = "select gratificacion_ncorr, fecha, trabajador, monto, caja, observacion
							from gratificaciones 
							where grupo = '".$_SESSION['gratificacion_grupo']."'";
			...
			while ($row=mysql_fetch_array($res_buscar)){
				...
				array_push($arrRegistros, array(
								...
								"fecha"			=> $row['fecha'],
								"codigo"		=> $row['gratificacion_ncorr']
								));
				$total = $total + $cantidad;
				}
			$miSmarty->assign('arrRegistros', $arrRegistros);
			$miSmarty->assign('total', $total);
			$miSmarty->assign('grupo', $_SESSION['gratificacion_grupo']);
			$objResponse->addAssign("divresultado", "innerHTML", $miSmarty->fetch('sg_gratificacion_ingresar_list.tpl'));	
...
function CargaPagina($data){
	...
	$objResponse->addAssign("grupo", "innerHTML", $_SESSION['gratificacion_grupo'] );
  	return $objResponse->getXML();
...
function Eliminar($data, $ncorr){
	...
	$sql_4 = "select grupo from sgvales.gratificaciones where gratificacion_ncorr = '".$ncorr."'";
	$res_4 = mysql_query($sql_4,$conexion) or die(mysql_error());
	$row_4 = mysql_fetch_array($res_4);
	$grupo = $row_4['grupo'];
	
	$sql_5 = "delete from sgvales.gratificaciones where gratificacion_ncorr = '".$ncorr."'";
	$res_5 = mysql_query($sql_5,$conexion) or die(mysql_error());
	
	$sql_buscar = "select gratificacion_ncorr, fecha, trabajador, monto, caja, observacion
							from sgvales.gratificaciones 
							where grupo = '".$grupo."'";
...
$miSmarty->display('sg_gratificacion_ingresar.tpl');
