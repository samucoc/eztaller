...
$xajax->setRequestURI("sg_alumnos_cobranza_pago_cheque_paso_2.php");
...
function CargaPagina($data){
	...
	$sql = "select * from gescolcl_arcoiris_administracion.Movimientos_Cheques where NumeroBoleta = '".$boleta."'";
	...
	$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_alumnos_cobranza_pago_cheque_paso_2_list.tpl'));
...
function LlamaMantenedorVxC($data){
	...
	$_SESSION["alycar_pagina_volver"]	=   ' sg_alumnos_cobranza_pago_cheque_paso_2.php';
...
function EfectuarPago($data){
	...
	$sql = "select * from gescolcl_arcoiris_administracion.Movimientos where NumeroBoleta in (".$boleta.")";
	...
	if (mysql_num_rows($res)>0){
		...
		$objResponse->addAlert("Boleta ya ocupada.");
...
$miSmarty->assign('boleta', $_GET['boleta']);

$miSmarty->assign('boleta_temp', $_GET['boleta_temp']);



$miSmarty->display('sg_alumnos_cobranza_pago_cheque_paso_2.tpl');
