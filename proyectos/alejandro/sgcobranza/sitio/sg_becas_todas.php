...
$xajax->setRequestURI("sg_becas_todas.php");
...
function Grabar($data){
	...
	if (($curso!='')&&($curso!='- - Elija - -')){
		...
		$and_alumno .= " and alumnos".$anio.".CodigoCurso = '".$curso."' ";
		...
		if ($alumno!=''){
			...
			$and_alumno .= " and alumnos".$anio.".NumeroRutAlumno IN (	select NumeroRutAlumno
	...
	$sql_pd = "select 
				...
				order by Cursos.CodigoCurso, alumnos".$anio.".PaternoAlumno"; 
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_row($res_pd)) {
		    ...
		    $cuota_0 = $row_001['ValorIncorporacion'];
			...
			array_push($arrRegistros, array("item"		=>	$i,
											...
											"ArancelIncorporacion"	=> 	$cuota_0,

											"ArancelColegiatura"	=> 	$colegiatura,
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_becas_todas_list.tpl'));
...
function LlamaDetalle($data, $rut_alumno){
	...
	$objResponse->addScript("showPopWin('sg_becas_todas_detalle.php?rut_alumno=$rut_alumno&curso=$curso', 'Asociar Beca', 1000, 500, null);");
...
$miSmarty->display('sg_becas_todas.tpl');
