...
function Buscar($data, $tabla, $campo1, $campo2){
	...
	if ($v_entidad == 12){
		...
		//$v_and 		= 	" and empresa_contr = '".$empresa."'";
		$tabla		=	"sggeneral.".$tabla;
	}
	if ($v_entidad == 4){
	...
	if ($v_buscar_por == '01'){
		...
		if ($v_entidad==12){
			$sql = "select $campo1, $campo2 
					from $tabla
					where (nombres like '%".$v_texto."%' ".$v_and.")
						or (apellido_pat  like '%".$v_texto."%' ".$v_and.")
						or (apellido_mat  like '%".$v_texto."%'	".$v_and.")";
			}
		/*
		if ($v_entidad == 8){
