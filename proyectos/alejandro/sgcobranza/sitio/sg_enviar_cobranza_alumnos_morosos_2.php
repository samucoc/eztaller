...
$xajax->setRequestURI("sg_enviar_cobranza_alumnos_morosos_2.php");
...
function Grabar($data){
    ...
    $email					 =		 $data['email']    ;
   	...
   	$objResponse->addScript("showPopWin('pdf_cobranza_alumnos_morosos_2.php?rut=$rut&fecha_buscar=$fecha_buscar&email=$email', 'Enviar PDF', 800, 600, null);");
...
function CargaPagina($data){
  ...
  //    	$sql_apoderado = "select concat(NombresApoderado,' ', PaternoApoderado,' ',MaternoApoderado) as nombre,
		...
		$sql_apoderado = "select  distinct concat(`PaternoAlumno`,' ',`MaternoAlumno`,' , ',`NombresAlumno`) as nombre_alumno,
									...
									alumnos".$anio.".`NumeroRutAlumno`, NroLista, NroMatricula, EMailAlumno
		...
		$objResponse->addAssign('email','value',$row_apoderado['EMailAlumno']);
...
$miSmarty->display('sg_enviar_cobranza_alumnos_morosos.tpl');
