...
$xajax->setRequestURI("sg_nylor_informe_precargas_2.php");
...
function CargaListado($data){
	...
	$res = mysql_query($sql, $conexion);
		...
		$and = "";
		if ($cod_vendedor !=''){
			$and = " and ve_codigo = '".$cod_vendedor."'  ";
			}
		...
		$arrRegistrosVend = ordenar_matriz_multidimensionada($arrRegistrosVend,'nombre','ASC'); 
		while ($line = mysql_fetch_row($res)) {
			...
			$sql_p = "select
					...
					vdet_ncorr in (select vdet_ncorr 
									from cargas_autorizadas 
			...
			while ($line_p = mysql_fetch_row($res_p)) {
				...
				$observacion="";
				while ($row_obs = mysql_fetch_array($res_obs)){
		...
		$arrRegistros_1		= 	array();
		$arrDetalle_1		= 	array();
		$arrRegistrosVend_1	= 	array();
		$and = "";
		if ($cod_vendedor !=''){
		$and = " and ve_codigo = '".$cod_vendedor."'  ";
		}
			...
			from sgbodega.vendedores
				...
				$res_nom_vend = mysql_query($sql_nom_vend,$conexion) or die(mysql_error());
				$nombre_vendedor = @mysql_result($res_nom_vend,0,"ve_vendedor");;
				array_push($arrRegistrosVend_1, array(
				"ncorr" => 	$ve_ncorr,
				"nombre"=>	$nombre_vendedor));
		
			}
			$arrRegistrosVend_1 = ordenar_matriz_multidimensionada($arrRegistrosVend_1,'nombre','ASC');
			while ($line = mysql_fetch_row($res)) {
				...
				if ($line[10] == 'A'){
				$estado_venta 	= 	'POR APROBAR';
				}	//#00CC00
					if ($line[10] == 'N'){
					$estado_venta 	= 	'NULA';
			}		//#FF0000
			if ($line[10] == 'B'){
				$estado_venta 	= 	'DE BAJA';
		}		//#FF99CC
		if ($line[10] == 'D'){
		$estado_venta 	= 	'DEVOLUCION';
		}	//#FF9900
		if ($line[10] == 'P'){
		$estado_venta 	= 	'PAGADA';
		}		//#0066FF
		...
		$dv= 0;
			...
			array_push($arrRegistros_1, array(
				"item" 			=> 	$i,
				"ncorr" 		=> 	$line[0],
				"folio" 		=> 	$line[1],
				"empresa"		=> 	$empresa,
		...
		$miSmarty->assign('arrRegistros_1', $arrRegistros_1);
		$miSmarty->assign('arrDetalle_1', $arrDetalle_1);
		$miSmarty->assign('arrRegistrosVend_1', $arrRegistrosVend_1);
		
		$objResponse->addAssign("divresultado", "innerHTML", $miSmarty->fetch('sg_nylor_informe_precargas_2_list.tpl'));
...
function AutorizaCarga($data, $folio){
	...
	$caut_folio 		= 	$folio;
	$caut_usuario 		= 	$_SESSION["alycar_usuario"];
...
function Pedido($data, $vent_ncorr, $codigo){
    global $conexion;
    $objResponse = new xajaxResponse('ISO-8859-1');
    $objResponse->addScript("showPopWin('sg_nylor_informe_precargas_2_pedidos.php?folio=$vent_ncorr&codigo=$codigo', 'Pedidos - Pendientes', 500, 250, null);");
    return $objResponse->getXML();
}
...
function Despacho($data, $vent_ncorr, $codigo){
    global $conexion;
    $objResponse = new xajaxResponse('ISO-8859-1');
    $objResponse->addScript("showPopWin('sg_nylor_informe_precargas_2_despacho.php?folio=$vent_ncorr&codigo=$codigo', 'Despachos', 500, 250, null);");
    return $objResponse->getXML();
}
...
function Aprobado($data, $vent_ncorr, $codigo){
    global $conexion;
    $objResponse = new xajaxResponse('ISO-8859-1');
    $objResponse->addScript("showPopWin('sg_nylor_informe_precargas_2_aprobado.php?folio=$vent_ncorr&codigo=$codigo', 'Aprobados', 600, 300, null);");
    return $objResponse->getXML();
}
...
$miSmarty->display('sg_nylor_informe_precargas_2.tpl');
