...
$xajax->setRequestURI("sg_bodega_cuentas_personales_2.php");
...
function Grabar($data){
	...
	if ($dias_dif < 0){
			...
			unset($_SESSION["alycar_sgyonley_cuentas_personales_2"]);
			$objResponse->addScript("window.document.Form1.submit();");
		}
		
	if ($ingresa == 'SI'){
		
		if (!isset($_SESSION["alycar_sgyonley_cuentas_personales_2"])){
			$sql = "insert into sgcompras.movim (movim_bodega,movim_tipo,empe_rut,movim_fecha,pr_rut,tdoc_ncorr,movim_numdoc,movim_obs,usu_id)
					values (
					'".$movim_bodega."','".$movim_tipo."','".$empe_rut."','".$movim_fecha."','".$pr_rut."','".$tdoc_ncorr."','".$movim_numdoc."',
					'".$movim_obs."','".$usu_id."')";
			
			$res = mysql_query($sql, $conexion);
			$ncorr = mysql_insert_id($conexion);
			
			$_SESSION["alycar_sgyonley_cuentas_personales_2"] = $ncorr;
		}
		
		// ingresa el articulo
		$movim_ncorr		=	$_SESSION["alycar_sgyonley_cuentas_personales_2"];
		
		$sql_det = "insert into sgcompras.movim_detalle (movim_ncorr,mdet_codigo,mdet_desc,mdet_nu,mdet_valor,mdet_cantidad,
					mdet_subneto,mdet_descuento,mdet_subtotal,movim_conf_bodega) 
					values ('".$movim_ncorr."','".$mdet_codigo."','".$mdet_desc."','".$mdet_nu."','".$mdet_valor."','".$mdet_cantidad."',
					'".$mdet_subneto."','".$mdet_descuento."','".$mdet_subtotal."','1')";
...
function ConfirmaIngreso($data){
	...
	if ($ingresa == 'SI'){
		...
		$sql_det = "select mdet_codigo, mdet_nu, mdet_valor, mdet_cantidad from sgcompras.movim_detalle where movim_ncorr = '".$movim_ncorr."'";
		$res_det = mysql_query($sql_det, $conexion);
		if (mysql_num_rows($res_det) > 0){
			while ($line_det = mysql_fetch_row($res_det)) {
				...
				$sql_tallas = "update sgcompras.tallas set
				...
				$sql = "select TA_STOCK1 as stock_nuevo, TA_STOCKUSA as stock_usado from sgcompras.tallas where TA_NCORR = '".$codigo."'";
				$res = mysql_query($sql, $conexion);
				if (mysql_num_rows($res) > 0){
					...
					$sql_upb = "update sgcompras.tallas set TA_STOCK1 = '".$stock_bodega_nuevo."', TA_STOCKUSA = '".$stock_bodega_usado."'
		...
		$objResponse->addScript("alert('Cuenta Personal ingresada Correctamente, El N� Asignado es: $movim_ncorr')");
...
function CargaListado($data){
	...
	$ncorr	=	$_SESSION["alycar_sgyonley_cuentas_personales_2"];
	...
	if (mysql_num_rows($res_ing) > 0){
		...
		// busca el proveedor
		$sql = "select pr_razon from sgcompras.proveedor where pr_rut = '".$pr_rut."'";
		$res = mysql_query($sql, $conexion);
	...
	if (mysql_num_rows($res) > 0){
		...
		$objResponse->addAssign("divresultadoarticulos", "innerHTML", $miSmarty->fetch('sg_bodega_cuentas_personales_2_list.tpl'));
...
$miSmarty->display('sg_bodega_cuentas_personales_2.tpl');
