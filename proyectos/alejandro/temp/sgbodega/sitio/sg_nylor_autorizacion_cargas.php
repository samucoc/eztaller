...
$xajax->setRequestURI("sg_nylor_autorizacion_cargas.php");
...
function CargaListado($data){
	...
	if ($v_inicio != '' && $v_termino != ''){
		...
		$sql = "select
				...
				where 
				vent_fecha >= '".$fecha1."' and vent_fecha <= '".$fecha2."' and
				$and				
		...
		if (mysql_num_rows($res) > 0){
			...
			$objResponse->addAssign("divresultado", "innerHTML", $miSmarty->fetch('sg_nylor_autorizacion_cargas_list.tpl'));
		...
		$objResponse->addScript("alert('Debe Indicar los Criterios de Busqueda.')");
	}
...
$miSmarty->display('sg_nylor_autorizacion_cargas.tpl');
