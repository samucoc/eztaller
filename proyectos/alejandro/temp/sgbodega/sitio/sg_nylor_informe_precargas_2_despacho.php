...
$xajax->setRequestURI("sg_nylor_informe_precargas_2_despacho.php");
...
function Grabar($data){
	...
	$guia_interna = $data['OBLIguia_interna'];
	...
	$fecha = $a.'-'.$m.'-'.$d;
		...
		if ($dias_diff >= 0 ){
			//$sql_busca = "select movim_ncorr from sgbodega.movim where movim_ncorr = '".$guia_interna."'";
//			$res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
//			
//			if (mysql_num_rows($res_busca)>0){
				$sql = "insert into sgyonley.cargas_despachadas(`folio`, `codigo`, `guia_interna`, `fecha_despacho`, `fecha_dig`, `usuario`) values ('".$folio."', '".$codigo."', '".$guia_interna."', '".$fecha."', '".date("Y-m-d H:i:s")."', '".$_SESSION["alycar_sgyonley_usuario"]."')";
			...
			//	} 
//			else{
//				$objResponse->addAlert("Guia Interna no Existe");
//				}
			}
		else{
			$objResponse->addAlert("Fecha de despcho anterior a fecha de rechazo");
			}
...
$miSmarty->display('sg_nylor_informe_precargas_2_despacho.tpl');
