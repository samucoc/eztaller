...
$xajax->setRequestURI("sg_nylor_informe_cargas_generadas.php");
...
function CargaListado($data){
	...
	if ($v_inicio != '' && $v_termino != ''){
		list($dia1,$mes1,$anio1) = split('[/.-]', $v_inicio);$fecha1 	= $anio1."-".$mes1."-".$dia1." 00:00:00";
		list($dia2,$mes2,$anio2) = split('[/.-]', $v_termino);$fecha2 	= $anio2."-".$mes2."-".$dia2." 23:59:59";
		...
		$sql = "select
				...
				EXISTS
				(SELECT crea_folio FROM cargas_realizadas 
				WHERE 
				crea_empresa = '".$empresa."' and 
				crea_folio = sgyonley.ventas_antigua.vent_num_folio and
				crea_fecha_dig >= '".$fecha1."' and crea_fecha_dig <= '".$fecha2."')
		...
		if (mysql_num_rows($res) > 0){
			...
			while ($line = mysql_fetch_row($res)) {
				...
				if (($filtro_estado == $estado_venta) OR ($filtro_estado == $estado_venta_doble) OR ($filtro_estado == 'TODAS')){
					...
					while ($line_p = mysql_fetch_row($res_p)) {
						...
						$sql_pau = "select caut_ncorr from sgyonley.cargas_autorizadas where caut_empresa = '".$empresa."' and caut_folio = '".$folio."' and vdet_ncorr = '".$vdet_ncorr."'";
			...
			$objResponse->addAssign("divresultado", "innerHTML", $miSmarty->fetch('sg_nylor_informe_cargas_generadas_list.tpl'));
			
			//llena div para la impresion del formulario de carga
			//$objResponse->addAssign("divresultadocarga", "innerHTML", $miSmarty->fetch('sg_nylor_informe_cargas_generadas_list.tpl'));
		...
		$objResponse->addScript("alert('Debe Indicar los Criterios de Búsqueda.')");
	}
...
function AutorizaCarga($data, $folio){
	...
	$sql = "insert into sgyonley.cargas_autorizadas (caut_empresa,caut_folio,caut_usuario,caut_fecha_dig)
...
function ConfirmaEliminaVenta($data, $ncorr){
	...
	sgyonley.$tabla_abonos	= 	"0".$_SESSION["alycar_sgyonley_empresa"]."_abonos".$sector;
...
$miSmarty->display('sg_nylor_informe_cargas_generadas.tpl');
