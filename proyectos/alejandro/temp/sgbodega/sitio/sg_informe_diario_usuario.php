...
$xajax->setRequestURI("sg_informe_diario_usuario.php");
...
function Grabar($data){
	...
	$sql = "select
				...
				from 
				sgcompras.movim a
					inner join sgcompras.movim_detalle c
	...
	if (mysql_num_rows($res) > 0){
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_diario_usuario_list.tpl'));
	}else{
...
function ImprimeGuia($data,$guia){
	...
	$_SESSION["alycar_pag_regreso"] = 'sg_informe_diario_usuario.php';
...
$miSmarty->display('sg_informe_diario_usuario.tpl');
