...
$xajax->setRequestURI("sg_bodega_aumentos_concon.php");
...
function ConfirmaIngreso($data){
	...
	if ($ingresa == 'SI'){
		if ($movim_numdoc == ''){
			$objResponse->addScript("alert('Nº Documento Incorrecto')");
			$ingresa = 'NO';
		}
	}
	if ($ingresa == 'SI'){
	...
	if ($ingresa == 'SI'){
		...
		$objResponse->addScript("alert('Aumento Ingresado Correctamente, El Nº Asignado es: $movim_ncorr')");
...
function CargaListado($data){
	...
	if (mysql_num_rows($res) > 0){
		...
		$objResponse->addAssign("divresultadoarticulos", "innerHTML", $miSmarty->fetch('sg_bodega_aumentos_concon_list.tpl'));
...
$miSmarty->display('sg_bodega_aumentos_concon.tpl');
