...
$xajax->setRequestURI("sg_nota_credito_ingreso_2.php");
		...
		if (mysql_num_rows($res_1)>0){
			if ($ingresa == 'SI'){
				...
				$sql = "insert into `notas_credito` ( `fecha`,  `empresa`, `rut`, `nro_nota_credito`, `nro_factura`, `neto`, `iva`, `total`, `observacion`, tipo_nc) values 
						('".$fecha."','".$empresa."','".$rut."','".$nro_nc."','".$nro_factura."','".$neto."','".$iva."','".$total."','".$observacion."', 'VENTAS')";
				$res = mysql_query($sql, $conexion);
...
function LlamaMantenedorVxC($data){
	...
	$_SESSION["alycar_pagina_volver"]	=   'sg_nota_credito_ingreso_2.php';
...
$miSmarty->display('sg_nota_credito_ingreso_2.tpl');
