...
$xajax->setRequestURI("sg_informe_nota_debito.php");
...
function Grabar($data){
	...
	if ($nc != ''){
		$and .= " and nro_nota_debito = '".$nc."' " ;
		}
    if ($tipo != ''){
		$and .= " and tipo_nd = '".$tipo."' " ;
		}
    if ($mes != ''){
			$and .= " and mes = '".$mes."' " ;
        	}
	...
	if (mysql_num_rows($res_ab) > 0){
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_nota_debito_list.tpl'));
...
function Eliminar($data,$ncorr){
	...
	$sql ="delete from nd_periodo where nro_nota_debito = ".$ncorr;
	$res = mysql_query($sql,$conexion);
...
$miSmarty->display('sg_informe_nota_debito.tpl');
