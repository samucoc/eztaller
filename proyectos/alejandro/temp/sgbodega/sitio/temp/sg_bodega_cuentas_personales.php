	...
	if (count($seleccion) > 0){
		...
		if ($ingresa == 'SI'){
			...
			foreach ($data['seleccion'] as $ncorr) {
				...
				//busca el nu antiguo
				$sql_na = "select sv_nu from sgbodega.sub_guiadev where sv_ncorr = '".$ncorr."'";
				$res_na = mysql_query($sql_na, $conexion);
				$nu_antiguo = @mysql_result($res_na,0,"sv_nu");
				$sql="";
				if ($nu=='N'){
					$sql = "update sgbodega.sub_guiadev set 
								sv_nu 			= 	'".$nu."', 
								sv_conf_bodega	=	'SI',
...
function CargaListado($data){
	...
	echo $sql_ve = "select 
				...
				from sgbodega.sub_guiadev a, sgbodega.d_guiadev b
	...
	*/
	...
	if (mysql_num_rows($res_ve) > 0){
		...
		while ($line_ve = mysql_fetch_row($res_ve)) {
			...
			/*
			// actualiza como confirmado en bodega los registros anteriores al año 2012
			// solicitado por Javier 22/08/2012
			$guia 	= $line_ve[2];
			$codbus = $line_ve[3];
			
			$sql_g = "delete from sgbodega.sub_guiadev where sv_guiadv = '".$guia."' and sv_codbus = '".$codbus."'";
			...
			*/
			...
			$guia = $line_ve[2];
			$sql_g = "select tdev_ncorr, gd_usuario, gd_fechadig from sgbodega.d_guiadev where gd_guia = '".$guia."' order by gd_ncorr desc limit 1";
			$res_g = mysql_query($sql_g, $conexion);
