	 ...
	 $sql = "select
		...
		if (mysql_num_rows($res) > 0){
			...
			while ($line = mysql_fetch_row($res)) {
				...
				if ($nombre_vendedor==''){
					$sql_vend = "select ve_vendedor from sgyonley.vendedores where ve_codigo = '".$vend_ncorr."' and ve_empresa = '".$empresa."'";
					$res_vend = mysql_query($sql_vend,$conexion);
					$nombre_vendedor = @mysql_result($res_vend,0,"ve_vendedor");
					}
				// busca el cliente
				...
				if (@mysql_num_rows($res_cli) < 1){
					$sql2 = "select vent_num_folio, sect_ncorr, clie_rut from ventas_antigua where clie_rut = '".$line[5]."' AND empe_rut = '".$empresa."' order by vent_ncorr desc";
					...
					while ($line2 = mysql_fetch_row($res2)) {
						if ($encontrado == 'NO'){
							...
							$sql_cl = "select clie_ncorr from $tabla_cli where clie_rut = '".$line[5]."'";
				...
				if (($filtro_estado == $estado_venta) OR ($filtro_estado == $estado_venta_doble) OR ($filtro_estado == 'TODAS')){
					...
					//echo $line[1].'->'.$valor_total.'->'.($valor_auto+$valor_elimi);
					//echo "<br>";
						if ($valor_total > ($valor_auto+$valor_elimi)){
							...
							$sql_p = "select
									...
									ventas_detalle_antigua
										left join ventas_detalle_obs
											on ventas_detalle_antigua.vdet_ncorr = ventas_detalle_obs.vent_det_ncorr
									WHERE
									vent_ncorr = '".$line[1]."' ";
							...
							while ($line_p = mysql_fetch_row($res_p)) {
								...
								if (mysql_num_rows($res_busqueda)==0){
									...
									if (mysql_num_rows($res_busqueda_1)==0){
												...
												$sql_pd = "select caut_fecha_dig as fecha_aut from cargas_autorizadas where vdet_ncorr = '".$vdet_ncorr."'";
...
function AutorizaCarga($data, $folio, $ncorr,$codigo){
	...
	$sql_pie = "select vent_pie, vent_total_venta from ventas_antigua where vent_num_folio = '".$folio."'";
	$res_pie = mysql_query($sql_pie,$conexion) or die(mysql_error());
	$row_pie = mysql_fetch_array($res_pie);
	
	if ($row_pie['vent_pie']==0){
		$objResponse->addScript("confirmacion = confirm('Confirma Autorizacion Carga?');
		...
		$objResponse->addScript("confirmacion = confirm('Confirma Autorizacion Carga?');
			...
			if (confirmacion == true)
			{
				confirmacion = confirm('Esta venta tiene un pie. Confirma Autorizacion Carga?');
				if (confirmacion == true){
					xajax_AutorizaCarga_1(xajax.getFormValues('Form1'),'$folio','$ncorr','$observacion');
					}
			}
...
function EliminaCarga($data, $folio, $ncorr,$codigo){
	...
	if ($row_pie['vent_pie']==0){
		$objResponse->addScript("confirmacion = confirm('Confirma Eliminacion Carga?');
			if (confirmacion == true)
			{
		...
		$objResponse->addScript("confirmacion = confirm('Confirma Eliminacion Carga?');
			if (confirmacion == true)
			{
				confirmacion = confirm('Esta venta tiene un pie. Confirma Eliminacion Carga?');
				if (confirmacion == true){
					xajax_EliminaCarga_1(xajax.getFormValues('Form1'),'$folio','$ncorr');
					}
				}
...
function AutorizaCarga_1($data, $folio, $ncorr,$observacion){
	...
	$sql = "insert into cargas_autorizadas (caut_empresa,caut_folio,vdet_ncorr,caut_usuario,caut_fecha_dig)
	...
	if ($row['observacion']==''){
		$sql = "insert into ventas_detalle_obs( `observacion`,`vent_det_ncorr`)
				values('".$observacion."', '".$ncorr."')";
		$res = mysql_query($sql,$conexion);
		}
	else{
		$sql = "update ventas_detalle_obs 
					set observacion = '".$observacion."'
					where vent_det_ncorr = '".$ncorr."'";
		$res = mysql_query($sql,$conexion);
		}
	// busca el producto
	$sql_pd = "select arti_codigo_largo as codigo, arti_desc as descripcion from ventas_detalle_antigua where vdet_ncorr = '".$ncorr."'";
	$res_pd = mysql_query($sql_pd,$conexion);
...
function EliminaCarga_1($data, $folio, $ncorr){
	...
	$sql = "insert into cargas_eliminadas (celi_empresa,celi_folio,vdet_ncorr,celi_usuario,celi_fecha_dig)
			values ('".$caut_empresa."','".$caut_folio."','".$ncorr."','".$caut_usuario."','".$caut_fecha_dig."')";
