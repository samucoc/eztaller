...
$xajax->setRequestURI("sg_mant_consulta_alumno.php");
...
function Grabar($data){
	...
	if ($ncorr == ''){
		...
		if (mysql_num_rows($res_c) > 0) {
			...
			$campos = $campos." where NumeroRutAlumno = '".$ncorr."'";
...
function CargaListado($data,$select,$tabla){
	...
	if (mysql_num_rows($res_c) > 0) {
		...
		$objResponse->addAssign("divresultado", "innerHTML", $miSmarty->fetch('sg_mant_consulta_alumno_list.tpl'));
		...
		if (mysql_num_rows($res) > 0){
			...
			while ($i < $j) {
				...
				elseif ($objeto=='OBLICodigoCurso'){
					...
					$sql_1 = "select CodigoCurso from gescolcl_nmva_administracion.Cursos where CodigoCurso like '%".$valor_campo."%'";
...
function CargaListado_alumnos_Notas($data){
	...
	if ($rut != ''){
		...
		$objResponse->addScript("location.href='sg_consulta_alumnos_notas.php?rut=".$rut."'");
...
function CargaListado_alumnos_HojaVida($data){
	...
	if ($rut != ''){
		...
		$objResponse->addScript("location.href='sg_consulta_alumnos_HojaVida.php?rut=".$rut."'");
...
function CargaListado_alumnos_Asistencia($data){
	...
	if ($rut != ''){
		...
		$objResponse->addScript("location.href='sg_consulta_alumnos_Asistencia.php?rut=".$rut."'");
...
function CargaListado_alumnos_Apoderado($data){
	...
	if ($rut != ''){
		...
		$objResponse->addScript("location.href='sg_consulta_alumnos_Apoderado.php?rut=".$rut."'");
...
$miSmarty->display('sg_mant_consulta_alumno.tpl');
