...
$pdf->AddPage();
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$rut."_notas_parciales.pdf","F");
	$path = 'pdfs/'.$fecha.'_'.$rut."_notas_parciales.pdf";  	
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Reporte Sistema Integrado de Gestion Escolar - Notas Parciales Alumno');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = "Envio de Notas Parciales Alumno";
