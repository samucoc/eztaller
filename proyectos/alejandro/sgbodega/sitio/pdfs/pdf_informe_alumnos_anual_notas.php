...
$pdf=new PDF_HTML('P','mm','legal'); //pagina carta horizontal
$pdf->AliasNbPages();
$pdf->AddPage();
	...
	while ($line_ve = mysql_fetch_row($res_ve)) {
			...
			$res_ex = mysql_query($sql_ex,$conexion) or die(mysql_error($conexion));
			$eximido = '0';
	...
	$pdf->Cell(160,5,'__________________________________________________________________________',0,'L');
	$pdf->SetXY($x,$y);	
	$pdf->Cell(175,5,utf8_decode(str_replace('Â°','°',utf8_decode($row_situacion['ObservacionSituacion'].'. '.substr($comentario_2,0,strlen($comentario_2)-1)))),0,'L');
	$pdf->Ln();
    $pdf->Cell(200,5,'_____________________________________________________________________________________________',0,'L');
	$pdf->Ln();
