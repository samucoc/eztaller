...
$pdf->SetAutoPageBreak(true,25);  
	...
	$sql_pd = "select 
				...
				IngresoFuncionario
				from Profesores
				where NumeroRutProfesor = '".$alumno."'"; 
	...
	while ($line_pd = mysql_fetch_row($res_pd)) {
		...
		$nombre_profesor = $line_pd[0];
		$rut = $line_pd[1].'-'.dv($line_pd[1]);
		$EspecialidadProfesor = $line_pd[2];
        $IngresoFuncionario = dias2letras($line_pd[3]);
	...
	$pdf->Cell(160,5,'CERTIFICADO LABORAL',0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	...
	$pdf->Row(array(utf8_decode('Josselyn Campos Sánchez, Directora del establecimiento educacional, '.$nombre_colegio.', RBD : '.$RBD.', de Villa Alemana, certifica que don (a) '.strtoupper($nombre_profesor).', R.U.N.: '.$rut.', trabaja en nuestro establecimiento como '.strtoupper($EspecialidadProfesor).' desde '.strtoupper($IngresoFuncionario).', con un contrato de carácter indefinido.')),0);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Row(array(utf8_decode('Se extiende el presente certificado a petición del trabajador(a) para '.utf8_encode($observacion))),0);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$alumno."_certificado_laboral.pdf","F");
	$path = 'pdfs/'.$fecha.'_'.$alumno."_certificado_laboral.pdf";  	
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Reporte Sistema Integrado de Gestion Escolar - Certificado Laboral');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = "Envio de Certificado Laboral";
