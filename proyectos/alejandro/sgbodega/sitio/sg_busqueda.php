...
function Buscar($data, $tabla, $campo1, $campo2){
	...
	if ($v_buscar_por == '01'){
		...
		if (($v_entidad == 4) OR ($v_entidad == 7) OR ($v_entidad == 'articulo')){
			$sql = "select ta_ncorr, concat(ta_busqueda, ' ', ta_descripcion) as descripcion from tallasnew where concat(ta_busqueda, '', ta_descripcion) like '%".$v_texto."%'";
		}
		
		/*
		if ($v_entidad == 8){
			$sql = "select b.ta_ncorr, a.bv_glosa, bv_valornuevo from 
					bodvendedor a,  tallasnew b where 
	...
	if ($v_buscar_por == '02'){
		...
		if (($v_entidad == 4) OR ($v_entidad == 7) OR ($v_entidad == 'articulo')){
			$sql = "select ta_ncorr, concat(ta_busqueda, ' ', ta_descripcion) as descripcion from tallasnew where ta_ncorr like '%".$v_texto."%'";
		}
	...
	if ($v_entidad == 8){
		if ($v_buscar_por == '01'){
			$sql_b = "select ta_ncorr, ta_busqueda,  ta_descripcion from tallasnew
