...
$xajax->setRequestURI("sg_informes_certificado_laboral.php");
...
function Grabar($data){
	...
	while ($line_pd = mysql_fetch_row($res_pd)) {
		...
		$miSmarty->assign('nombre_profesor',$line_pd[0]);
		$miSmarty->assign('rut',$line_pd[1].'-'.dv($line_pd[1]));
		$miSmarty->assign('EspecialidadProfesor',$line_pd[2]);
		$miSmarty->assign('IngresoFuncionario',dias2letras($line_pd[3]));

	}
	$miSmarty->assign('observacion',$observacion);
	$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informes_certificado_laboral_list.tpl'));	
...
function GenerarPDF($data){
    ...
    $cert_1 = $data['cert_1'] == '1' ? $data['cert_1'] : '0';
    $cert_2 = $data['cert_2'] == '1' ? $data['cert_2'] : '0';
    $cert_3 = $data['cert_3'] == '1' ? $data['cert_3'] : '0';
    $cert_4 = $data['cert_4'] == '1' ? $data['cert_4'] : '0';
    $cert_5 = $data['cert_5'] == '1' ? $data['cert_5'] : '0';
    $cert_6 = $data['cert_6'] == '1' ? $data['cert_6'] : '0';
    $cert_7 = $data['cert_7'] == '1' ? $data['cert_7'] : '0';
    $cert_8 = $data['cert_8'] == '1' ? $data['cert_8'] : '0';
    $cert_9 = $data['cert_9'] == '1' ? $data['cert_9'] : '0';
    $cert_0 = $data['cert_0'] == '1' ? $data['cert_0'] : '0';
    
    $certs = $cert_1.'_'.$cert_2.'_'.$cert_3.'_'.$cert_4.'_';
	$certs .= $cert_5.'_'.$cert_6.'_'.$cert_7.'_'.$cert_8.'_';
	$certs .= $cert_9.'_'.$cert_0;

	$objResponse->addScript("showPopWin('pdfs/pdf_generar_certificado_laboral.php?alumno=$alumno&observacion=$observacion&certificados=$certs', 'Imprime PDF', 1000, 700, null);");
...
function EnivarPDF($data){
	...
	$objResponse->addScript("showPopWin('sg_envia_certificado_laboral.php?alumno=$alumno&observacion=$observacion', 'Enviar PDF', 600, 200, null);");
...
$miSmarty->display('sg_informes_certificado_laboral.tpl');
