...
$pdf->AddPage();
	...
	while($row_alumno_busca = mysql_fetch_array($res_alumno_busca)){
		...
		if ($semestre==1){
			$nombre_semestre = 'Primer Semestre';
		 	}
		else{
			$nombre_semestre = 'Segundo Semestre';
		 	}
		...
		$y = $pdf->GetY();
		$pdf->SetXY($x,$y);	
		$pdf->Cell(200,5,'INFORME FINAL',0,0,'C');
		$pdf->Ln();
		$pdf->SetFont('Arial','B',8);
		$pdf->Cell(200,5,$nombre_semestre.'-'.$anio,0,0,'C');
		$pdf->Ln();
		$pdf->Ln();
...
$sql_ve = "select  distinct concat(`PaternoAlumno`,' ',`MaternoAlumno`,' , ',`NombresAlumno`) as nombre_alumno,
    ...
    $pdf->Ln();
		...
		$sql_asistencia = "SELECT DiasPeriodo as cantidad, TerminoPeriodo
							from Periodos 
		...
		$TerminoPeriodo = $row_asistencia['TerminoPeriodo'];
...
$pdf->Row(array(utf8_decode('____________________________________________________________________________________________________')),0);
$pdf->Row(array(utf8_decode('____________________________________________________________________________________________________')),0);
$pdf->Row(array(utf8_decode('____________________________________________________________________________________________________')),0);
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$semestre.'_'.$curso."_notas_semestral.pdf","F");
	$path = 'pdfs/'.$fecha.'_'.$semestre.'_'.$curso."_notas_semestral.pdf";  	
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Reporte Sistema Integrado de Gestion Escolar - Notas Semestral Curso');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = "Envio de Notas Semestral Curso";
