...
$xajax->setRequestURI("sg_matricula_ingresar_nuevo.php");
...
function CargaPagina($data){
	...
	// carga empresas
	$where = '"%ADMISION%"';
	$objResponse->addScript("xajax_CargaSelect(xajax.getFormValues('Form1'),'curso','Cursos','','- - Seleccione - -','CodigoCurso','NombreCurso', ' where NombreCurso like ".$where."')");
...
function CargaListado($data){
	...
	if (mysql_num_rows($res_ve) > 0){
		...
		while ($line_ve = mysql_fetch_row($res_ve)) {
			$select_alumnos = "select Matriculado from alumnos".$anio_sup." where NumeroRutAlumno = '".$line_ve[1]."'";
			$res_alumnos = mysql_query($select_alumnos,$conexion);
			$row_alumnos = mysql_fetch_array($res_alumnos);
			$autorizado = '0';
			if (mysql_num_rows($res_alumnos)>0){
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_matricula_ingresar_nuevo_list.tpl'));
...
function ActulizarAlumno($data,$rut_alumno){
	...
	$objResponse->addScript("showPopWin('sg_matricula_actualizar_nuevo.php?rut_alumno=".$rut_alumno."', 'Autoriza Postulantes Nuevos', 1000, 240, null);");
...
$miSmarty->display('sg_matricula_ingresar_nuevo.tpl');
