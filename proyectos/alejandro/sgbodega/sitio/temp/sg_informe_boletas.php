...
$xajax->setRequestURI("sg_informe_boletas.php");
...
function Grabar($data){
	...
	if (mysql_num_rows($res_ab) > 0){
		...
		while ($line_ab_1 = mysql_fetch_array($res_ab)){
			...
			$nombre_persona = @mysql_result($res_pr,0,"empe_desc");
			$total += $line_ab_1['monto'];
			...
			array_push($arrRegistros, array("item"			=>	$i,
											"ncorr" 		=> 	$line_ab_1['boleta_ncorr'],
											"empresa"  		=> 	$nombre_persona,
											"fecha"   		=> 	$line_ab_1['fecha'],
											"mes_contable"  => 	$line_ab_1['mes']."-".$line_ab_1['anio'],
											"nro_boleta"   	=> 	$line_ab_1['nro_boleta'],
											"monto"   		=> 	$line_ab_1['monto'],
											"cierre"		=>  $ingresa 
											));
			$i++;
		...
		$objResponse->addAssign("total", "innerHTML", $total);
...
$miSmarty->display('sg_informe_boletas.tpl');
