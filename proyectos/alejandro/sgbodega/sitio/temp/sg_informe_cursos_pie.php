...
$xajax->setRequestURI("sg_informe_cursos_pie.php");
...
function Grabar($data){
	...
	$sql_pd = "select 
				...
				alumnos".$anio.".EvaluacionDiferenciada,

				alumnos".$anio.".TextEvaluacionDiferenciada,

				alumnos".$anio.".DiagnosticoAprendizaje,

				alumnos".$anio.".TextDiagnosticoAprendizaje
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_row($res_pd)) {
			...
			array_push($arrRegistros, array("item"			=>	$i,
											...
											"EvaluacionDiferenciada"	=> 	$line_pd[9],

											"TextEvaluacionDiferenciada"	=> 	$line_pd[10]));
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_cursos_pie_list.tpl'));
...
$miSmarty->display('sg_informe_cursos_pie.tpl');
