...
function RegistroGrabado() {
  ...
  document.location = 'cobranza_tipo_pagos.php';
}
 //-->
</script>
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">Tipos de pagos</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS TIPOS pagos</span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td><form action="cobranza_tipo_pagos.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
			...
			function mensaje()
				{
					echo "<script>
					alert('Debe Ingresar tipo de pagos');
					</script>";
				}
		   ?>
          </table>
        <table class="sortable" id="unique_id" width="100%" border="0" align="center" cellpadding="1" cellspacing="1" >
			...
			$sql="SELECT * FROM  tipo_pagos order by tp_ncorr";
		 ...
		 ?></th>
            ...
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bgcolor="#FFFFFF">
			  <?php 
			       $cantidad = strlen($registro['tp_ncorr']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['tp_ncorr'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['tp_ncorr'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['tp_ncorr'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['tp_ncorr'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['tp_ncorr'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['tp_ncorr'] . ' ';  }
			  ?>
			  </td>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM tipo_pagos WHERE tp_ncorr =".$_POST['txt_cod'];
				...
				if (empty($_POST['txt_familia'])){  
					$link=mensaje();
					   ...
					   $codigo=(string)(int)$codigo;
						...
						if (empty($codigo)){
						   mysql_query("insert into tipo_pagos (nombre) values ('$nom_familia')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";					
					  } else {
							$sql = "UPDATE tipo_pagos SET nombre='$nom_familia' where tp_ncorr='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
