...
function RegistroGrabado() {
  ...
  document.location = 'cobranza_tipo_diagnostico.php';
}
 //-->
</script>
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">Tipos de diagnosticos</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS TIPOS diagnosticos</span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td><form action="cobranza_tipo_diagnostico.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
			...
			function mensaje()
				{
					echo "<script>
					alert('Debe Ingresar tipo de diagnosticos');
					</script>";
				}
		   ?>
          </table>
        <table class="sortable" id="unique_id" width="100%" border="0" align="center" cellpadding="1" cellspacing="1" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th width="29%" bgcolor="#06327D"><span class="Estilo17">C&oacute;digo Tipo diagnostico</span></th>
              <th width="33%" bgcolor="#06327D" class="CONT"><span class="Estilo17">Nombre</span></th>
              <th width="12%" bgcolor="#06327D" class="CONT"><?php
			$sql="SELECT * FROM  tipo_diagnosticos order by td_ncorr";
		 ...
		 ?></th>
            ...
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bgcolor="#FFFFFF">
			  <?php 
			       $cantidad = strlen($registro['td_ncorr']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['td_ncorr'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['td_ncorr'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['td_ncorr'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['td_ncorr'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['td_ncorr'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['td_ncorr'] . ' ';  }
			  ?>
			  </td>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM tipo_diagnosticos WHERE td_ncorr =".$_POST['txt_cod'];
				...
				if (empty($_POST['txt_familia'])){  
					$link=mensaje();
					   ...
					   $codigo=(string)(int)$codigo;
						...
						if (empty($codigo)){
						   mysql_query("insert into tipo_diagnosticos (nombre) values ('$nom_familia')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";					
					  } else {
							$sql = "UPDATE tipo_diagnosticos SET nombre='$nom_familia' where td_ncorr='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
