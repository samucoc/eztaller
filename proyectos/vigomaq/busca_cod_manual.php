		  ...
		  if ($_POST['OK']=='Guardar y Seguir')
		 {
		   ...
		   $codigo       = trim($_POST['txt_cod']);
			...
			if (!empty($cod_arriendo))
			{
				...
				mysql_query("insert into equipos_arriendo_temp (cod_arriendo, cod_equipo, estado_equipo_arr,usuario) values ('$cod_arriendo', '$codigo', 'NO DEVUELTO','$usuario')",$link);

				//cambiar estado equipo
				//$sql = "UPDATE equipo SET cod_estado_equipo='3' where cod_equipo='$codigo'";
				//$res  = mysql_query($sql) or die(mysql_error());
				mysql_close($link);	  
		        echo "<script language=Javascript> location.href=\"arriendo_equipo_manual.php?codarr=".$cod_arriendo."\"; </script>";
			} else {
				$link=mensaje();
			}
		 }
