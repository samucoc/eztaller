    ...
    ?>
      ...
      echo ($row["nombres_personal"].' '.$row["ap_patpersonal"]);?>" size="30" maxlength="30" />          </td>
        ...
        <tr>
          ...
          <td align="left"><input name="txt_oc" id="txt_oc" type="text" value="<?php if (!empty($registro['num_oc'])){echo ($registro['num_oc']);}else{echo($_POST["txt_oc"]) ;}?>" size="20" maxlength="20" /></td>
     ...
     ?>
        ...
        <tr>
          ...
          <td colspan="4" align="right"><span class="Estilo71">
            ...
            <a href='arriendo_equipo_manual.php?codarr=<?php echo $registro['cod_arriendo'] ?>'>
              ...
              if (empty($oc)||empty($tipo_oc)||empty($num_gd)||empty($tipo_garantia)||empty($forma_pago)||empty($vendedor)||empty($fecha_inicio)||empty($fecha_arr)||empty($hora_arr)||empty($forma_entrega))
              {
                ...
                }elseif ($registrof['estado']=='NULA')
                {
                  echo "<script> alert (\"Guia despacho nula. Ingrese Otro Folio\"); </script>";
                    ...
                    $sql = "insert into arriendo (rut_cliente,cod_obra,num_gd,num_oc,tipo_oc,fecha_vcmto,tipo_garantia,forma_pago,cod_personal,fecha_inicio,fecha_arr,hora_arr,forma_entrega)
          values ('$rut_cliente','$cod_obra','$num_gd','$num_oc','$tipo_oc','$vcmto_oc','$tipo_garantia','$forma_pago','$vendedor','$fecha_inicio','$fecha_arr','$hora_arr','$forma_entrega')";
                    $res = mysql_query($sql) or die(mysql_error());
                    //echo "<br/>";
                    ...
                    //echo "<br/>";
                    mysql_query("insert into gd (id_arriendo,num_gd,cod_cliente,fecha,tipo,rut_cliente,cod_obra,patente) values ('$ultimo_cod_arriendo','$num_gd','$codigocli','$fecha_emision_gd','$tipo','$rut_cli','$nom_obrarr','$patente')",$link);
                    ...
                    while ($row = mysql_fetch_array($res)){
                      ...
                      $sql_1 = "insert into equipos_arriendo (cod_arriendo, cod_equipo, estado_equipo_arr,num_gd,arrendado_desde,hora_arr,precio) 
                      values ('$ultimo_cod_arriendo','$codigo', 'NO DEVUELTO','$num_gd','$fecha_arr','$hora_arr','$precio')";
                  ...
                  echo "<script language=Javascript> location.href=\"doc_arriendo_manual.php?codarr=".$ultimo_cod_arriendo."\"; </script>";
                  }
                }
              }
            }
