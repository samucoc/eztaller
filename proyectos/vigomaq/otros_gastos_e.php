...
function Noingresado() {
  alert("Equipo no ha sido ingresado!");
  document.location = 'otros_gastos_e.php';
}
 //-->
</script>
...
<table width="95%" border="0" align="center">
  <tr>
    <td width="80%" height="80" align="center" valign="top"><form action="otros_gastos_e.php" method="POST" name="frmDatos" id="frmDatos">
	 ...
	 ?>
              <?php
			{
				if ($_GET['codigo'] != "")
				{
				   $txt_codigo = $_GET["codigo"];
					...
					$sql = "SELECT * FROM equipo where cod_equipo ='$txt_codigo'";
		...
		?>
              </span>AJUSTE RENTABILIDAD EQUIPO</font></strong></div>
          </div></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="4" bgcolor="#06327D"><div align="left"><span class="Estilo7 Estilo25">BUSSQUEDA EQUIPO</span></div></td>
        </tr>
        <tr>
          <td colspan="4"></td>
        </tr>
        <tr class="bord_img">
          ...
          <td colspan="2"><input  name="txt_codigo" type="text" size="8" maxlength="8" value=""/>
            <input type="submit" name="buscarcodigo" title="Buscar Equipo por Codigo" value="Buscar" style="background-image:url(images/ver.png); width:16px; height:16px;" class="formato_boton" />
            ...
            <?php
				...
				if (($_POST['buscarcodigo']=='Buscar'))
				{
					...
					echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=busca_eq_otroe.php?id=$busca_cod'>";
				}
				//envia el codigo
				if (($_POST['buscarnombre']=='Buscar'))
				{
					...
					echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=busca_eq_otroe.php?nombre=$busca_nom'>";
				}
			?>
            &nbsp;</td>
        </tr>
        ...
        <tr>
          <td colspan="4" bgcolor="#06327D"><div align="left"><span class="Estilo7 Estilo25">DATOS AJUSTE RENTABILIDAD</span></div></td>
        </tr>
        <tr>
          <td><div align="left">C&oacute;digo Equipo</div></td>
          <td>&nbsp;</td>
          <td><input  name="txt_codigo2" type="text" size="8" maxlength="8" value="<?php echo $registro["cod_equipo"] ?>" disabled="disabled"/>
            <input type="hidden" name="txt_cod" size="20" maxlength="30" value="<?php echo $registro["cod_equipo"]?>"/></td>
          <td>&nbsp;</td>
        </tr>
        <tr>
          <td><div align="left">Nombre Equipo </div></td>
          <td>:</td>
          <td><div align="left">
            <input name="txt_nombre_equ2" type="text" value="<?php echo htmlentities($registro["nombre_equipo"]) ?>" size="35" maxlength="35" disabled="disabled"/>
          </div></td>
          <td>&nbsp;</td>
        </tr>
        <tr>
        ...
        <tr>
          <td><div align="left">Tipo Ajuste</div></td>
          <td>:</td>
          <td>
          	<div align="left">
            	Egreso 
            	  <input type="radio" name="tipo" value="egreso" id="tipo_egreso" />
                Ingreso 
                <input type="radio" name="tipo" value="ingreso" id="tipo_ingreso" />
            </div>
          </td>
          <td>&nbsp;</td>
        </tr>
...
if ($_POST['OK']=='Guardar y Seguir') {
    $codigo_equipo = $_POST['txt_cod']; // echo "$codigo_equipo<br>";
	...
	if (empty($codigo_equipo)||empty($fecha_gasto)||empty($Observaciones)){  
		$link=mensaje();
	} else {

		$fecha_gasto        = tranf_fecha_1($_POST['cal-field-1']); 	            //echo "$fecha_gasto<br>";
		$monto_gasto        = $_POST['txt_monto'];                 //echo "$monto_gasto<br>";
		$Observaciones      = strtoupper($_POST['txt_obsrepos']);  //echo "$Observaciones<br>";
		$Observaciones      = htmlspecialchars_decode(trim($Observaciones));
		$tipo_ajuste		= $_POST['tipo'];
		
		//echo "insert into vigomaq_intranet.otros_gastos_e (cod_equipo,fecha_gasto,monto_gasto,Observaciones,tipo_ajuste) values ('$codigo_equipo','$fecha_gasto','$monto_gasto','$Observaciones','$tipo_ajuste')";
		mysql_query("insert into vigomaq_intranet.otros_gastos_e (cod_equipo,fecha_gasto,monto_gasto,Observaciones,tipo_ajuste) values ('$codigo_equipo','$fecha_gasto','$monto_gasto','$Observaciones','$tipo_ajuste')",$link);
		 
		echo "<script> alert (\"Proceso realizado con Exito.\"); </script>";
		echo "<script language=Javascript> location.href=\"otros_gastos_e.php\"; </script>"; 
		} 
	}
