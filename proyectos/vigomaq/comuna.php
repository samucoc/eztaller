...
<table width="98%" border="0">
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">COMUNAS</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS COMUNA </span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td><form action="comuna.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
            <tr>
              <td width="138" class='mini_titulo'><div align="left"><font><strong>Comuna :</strong></font></div></td>
              <td width="182"  class='mini_titulo'><font>
                <input type="hidden" name="txt_cod" size="20" maxlength="30" > <input type="text" name="txt_comuna" size="25" maxlength="25" />
              </font> </td>
        ...
        <table class="sortable" id="unique_id" width="100%" border="0" align="center" cellpadding="1" cellspacing="1" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th width="38%" bgcolor="#06327D"><span class="Estilo17">C&oacute;digo Comuna </span></th>
              <th width="49%" bgcolor="#06327D" class="CONT"><span class="Estilo17">Comuna</span></th>
              <th width="13%" bgcolor="#06327D" class="CONT">
              <span class="Estilo17 Estilo13 Estilo15">Eliminar</span></th>
            </tr>
            <?php
			$sql="SELECT * FROM comuna order by cod_comuna";
		 ...
		 ?>
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bgcolor="#FFFFFF">
			  <?php
				   $cantidad = strlen($registro['cod_comuna']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['cod_comuna'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['cod_comuna'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['cod_comuna'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['cod_comuna'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['cod_comuna'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['cod_comuna'] . ' ';  }		   
	      	 	?>
			  </td>
              <td bgcolor="#FFFFFF"><?php echo '' . $registro['comuna'] . ' ';?></td>
              <td align="center" bgcolor="#FFFFFF">
              
              <input type="submit" name="borrar" value="Borrar" title="Eliminar Comuna" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> style="background-image:url(images/error.png); width:16px; height:16px;" class="formato_boton" />
              
              <!--<input type="image" name="borrar" value="Borrar" title="Eliminar Comuna" src="images/error.png" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> />
              -->
              </td>
          </tr>
            <tr>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM comuna WHERE cod_comuna =".$_POST['txt_cod'];
	 ...
	 ?>
		 ...
		 $link=Conectarse();
		  if($_POST['OK']=='Guardar y Seguir'){
				if (empty($_POST[txt_comuna])){  
					$link=mensaje();
				} else {  
					   $codigo  = $_POST['txt_cod'];
					   $comuna  = strtoupper($_POST['txt_comuna']);	
					   $codigo  = (string)(int)$codigo;
						if (empty($codigo)){
						   mysql_query("insert into comuna (comuna) value ('$comuna')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";						
					  } else {
							$sql = "UPDATE comuna SET comuna='$comuna' where cod_comuna='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
  ...
  <br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=comuna.php'>";?>
</div>
