...
$sql="SELECT * FROM personal order by rut_personal ASC LIMIT ".$limitInf.",".$tamPag;
$res=mysql_query($sql);
...
?>
            ...
            <td><?php 
			if (!empty($registro['cod_tipo_pers']))
			  {
				  $sql3="SELECT tipo_personal FROM tipo_personal where cod_tipo_pers =".$registro['cod_tipo_pers'];
				 
				  $res3 = mysql_query($sql3,$link) or die(mysql_error()); 
				  $registro3 = mysql_fetch_array($res3);
				  echo($registro3['tipo_personal']);
			  }else{
				  echo(" ");
			  }
			?></td>
