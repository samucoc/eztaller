...
function RegistroGrabado() {
  ...
  document.location = 'familia_equipo.php';
}
 //-->
</script>
...
<table width="98%" border="0">
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">FAMILIA  EQUIPOS </div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS FAMILIA EQUIPOS </span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td><form action="familia_equipo.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
			...
			function mensaje()
				{
					echo "<script>
					alert('Debe Ingresar Familia de Equipo');
					</script>";
				}
		   ?>
          </table>
        <table class="sortable" id="unique_id" width="100%" border="0" align="center" cellpadding="1" cellspacing="1" >
			...
			$sql="SELECT * FROM  tipo_familia_equipo order by id_tipo";
		 ...
		 ?></th>
            ...
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bgcolor="#FFFFFF">
			  <?php 
			       $cantidad = strlen($registro['id_tipo']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['id_tipo'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['id_tipo'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['id_tipo'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['id_tipo'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['id_tipo'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['id_tipo'] . ' ';  }
			  ?>
			  </td>
              <td bgcolor="#FFFFFF"><?php echo '' . utf8_encode($registro['nombre']) . ' ';?></td>
              <td bgcolor="#FFFFFF"><?php echo '' . utf8_encode($registro['descripcion']) . ' ';?></td>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM tipo_familia_equipo WHERE id_tipo =".$_POST['txt_cod'];
				...
				if (empty($_POST[txt_familia])){  
					$link=mensaje();
					   ...
					   $codigo=(string)(int)$codigo;
						...
						if (empty($codigo)){
						   mysql_query("insert into vigomaq_intranet.tipo_familia_equipo (nombre,descripcion) values ('$nom_familia', '$descripcion')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";					
					  } else {
							$sql = "UPDATE vigomaq_intranet.tipo_familia_equipo SET nombre='$nom_familia', descripcion = '$descripcion' where id_tipo='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
  ...
  <br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=familia_equipo.php'>";?>
</div>
