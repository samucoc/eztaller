...
while($row_ob_eq = mysql_fetch_array($res_ob_eq)){
	...
	$sql_factura ="select distinct factura.num_factura
				from factura
				...
				where factura.fecha between '".$inicio."' and '".$fin."'
					...
					and factura.observaciones like '%".$row_ob_eq['num_gd']."%'
				order by factura.num_factura";
	$res_factura = mysql_query($sql_factura,$link) or die(mysql_error());
	$row_factura = mysql_fetch_array($res_factura);
	
	echo "<td>".$row_factura['num_factura']."</td>";
