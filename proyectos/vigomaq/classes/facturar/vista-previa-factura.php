...
$pdf->AddPage();
	...
	if (empty($valor1)){
			...
			$sqlact = "UPDATE factura SET estado ='CERRADA' where num_factura='$factura'";
	...
	$sqldet_1="SELECT count(num_factura) FROM  det_factura where num_factura = '".$factura."' ";	
		...
		if (($registrodet['cod_repuesto'] == 0 )&&($registrodet['cod_equipo'] == 0 )) {
			...
			$xmlstr .=			'</Encabezado>
									<Detalle>
				  					        ...
				  					        <NmbItem>'.substr($detalle,0,80).'</NmbItem>
			...
			if (!empty($registrodet['total_rep'])) { 
				...
				if ($i == $contador)	$detalle_1 .= ' Nombre Obra : '.$nombre_obra.' - Direccion Obra : '.$direcc_obra;
				$xmlstr .=			'<Detalle>
				  					        <NroLinDet>'.$i.'</NroLinDet>
				  					        <NmbItem>'.str_replace("Ã‘",'N',substr($detalle,0,80)).'</NmbItem>
				  					        <DscItem>'.$detalle_1.'</DscItem>
	...
	header('Content-Disposition: attachment; filename="R76836180-0T33F'.$factura.'.xml"');
	header ("Content-Type: application/force-download");
	fopen($archivo,$xml->asXML());
...
// 	$pdf->SetXY(130,51.5);
// 	$pdf->Cell(50,5,$fecha_factura_imprimir.' - arial');
// 	$pdf->Ln();
...
// 			}
// 			$pdf->Cell(100,6,iconv('UTF-8', 'windows-1252',iconv('UTF-8', 'windows-1252',(utf8_decode($detalle)))));			  
// 			$link=Conectarse();
// 			$sqleval   = "SELECT valor_unidad_arr FROM equipo WHERE cod_equipo =".$registrodet['cod_equipo'];
// 			$resuni         = mysql_query($sqleval,$link) or die(mysql_error()); 
// 			$registroval    = mysql_fetch_array($resuni);
...
// 			$pdf->Ln();
// 			$pdf->Cell(30);
// 			$pdf->Cell(100,6,iconv('UTF-8', 'windows-1252',iconv('UTF-8', 'windows-1252',(utf8_decode($detalle_1)))),0,0,'L');
// 			$pdf->Ln();
// 		}
// 	}
