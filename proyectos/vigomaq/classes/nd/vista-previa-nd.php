...
include("../conex.php");
	...
	class PDF extends FPDF{
			...
			function Header(){
				// Logo
				//$this->Image('../../images/fondo_nd.png',0,0,215);
				}
		...
		if (empty($num_nd)) $num_nd = $_GET['num_nd'];
		$link=Conectarse();
		
		$sqlguia = "SELECT * FROM nota_debito WHERE num_nota_cred ='$num_nd'";						
		
		$resguia = mysql_query($sqlguia,$link) or die(mysql_error()); 
		$registronc = mysql_fetch_array($resguia);

		$num_nd       	=	$registronc['num_nota_cred'];
		$cod_cli  		=	$registronc['cod_cliente'];
		$num_cred 		= 	$registronc['num_factura'];
		$num_factura 	= 	$registronc['num_factura'];
		$fecha 			= 	$registronc['fecha'];

 		  $sqlfactura     = "SELECT * from nota_credito where num_nota_cred ='$num_factura'";
          
          $resfactura          = mysql_query($sqlfactura,$link) or die(mysql_error()); 
          $registrofactura    = mysql_fetch_array($resfactura);
          $num_factura        = $registrofactura['num_factura'];
          $fecha_nc 	       = $registrofactura['fecha'];
          ...
          $registrocliente 	= mysql_fetch_array($rescli);
          $valor1			=	$registrocliente['rut_cliente'];
		  $cliente     		= $registrocliente['raz_social'];
	...
	$xmlstr = '<?xml version="1.0" encoding="utf-8"?>
				<DTE version="1.0">
					<Documento ID="R76836180-0T56F'.$num_nd.'">
						<Encabezado>
							<IdDoc>
			            		<TipoDTE>56</TipoDTE>
					            <Folio>'.$num_nd.'</Folio>
	...
	while ($registrodet = mysql_fetch_array($resdet)) {
		...
		$xmlstr .=			'<Detalle>
	  					        ...
	  					        <NmbItem>'.substr(str_replace("Ã‘",'N','Nota de Débito que corresponde a :'),0,80).'</NmbItem>
	...
	$row_fact = mysql_fetch_array($res_fact);
		...
		$sql_dte = "select * 
					from folios_dte 
					where (tipo ='61' and desde <= '".$num_factura."') ";
		$res_dte = mysql_query($sql_dte,$link);
	
		if (mysql_num_rows($res_dte)>0){
		    $sqlnc     = "SELECT * FROM nota_credito WHERE num_nota_cred ='$num_cred'";
			...
			$xmlstr .=' <Referencia>
							...
							<FolioRef>'.$num_cred.'</FolioRef>
							<FchRef>'.$registrofactura['fecha'].'</FchRef>
	...
	header("Content-Disposition: attachment; filename=nd_".$num_nd.".xml");
...
// 		$pdf->Ln();
// 		$tot_nd = $tot_nd + ($registrodet['cantidad']*$registrodet['monto']);
// 		}
		
// 	$pdf->SetXY(180,140);
// 	$pdf->Cell(50,5,$tot_nd);
