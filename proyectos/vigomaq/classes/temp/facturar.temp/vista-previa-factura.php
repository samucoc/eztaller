...
$pdf->AddPage();
	...
	if (!empty($registro['cod_comuna'])) {
		...
		$pdf->SetXY(120,68.5);
		$pdf->Cell(50,5,utf8_decode_1($registrociu['comuna']));
	  }else{;
		$pdf->SetXY(120,68.5);
		$pdf->Cell(50,5," ");
	  } ;
	...
	$sqlgd   = "SELECT * FROM vigomaq_intranet.arriendo WHERE cod_arriendo ='$num_arriendo'";
	...
	if ((($registrogd['num_gd'])!='')&&(($registrogd['num_gd'])!=0)) {
		...
		$sqlgd   = "SELECT distinct num_gd FROM vigomaq_intranet.equipos_arriendo WHERE cod_arriendo ='$num_arriendo'";
	...
	if (!empty($registropago['forma_pago'])){
		...
		for ($i=0; $i<=($total_string/15);$i++){
			$pdf->Cell(15,5,utf8_decode_1(utf8_decode_1(substr($registropago['forma_pago'],$i*15,15))));
			$pdf->Ln();
			$pdf->Cell(165,5,"");
			}
		$pdf->Cell(15,5,utf8_decode_1(utf8_decode_1(substr($registropago['forma_pago'],$i*15,15)))) ;
		...
		if (!empty($registropago['cond_venta'])){
			...
			for ($i=0; $i<=($total_string/15);$i++){
				$pdf->Cell(15,5,utf8_decode_1(utf8_decode_1(substr($registropago['cond_venta'],$i*15,15))));
				$pdf->Ln();
				$pdf->Cell(165,5,"");
				}
			$pdf->Cell(15,5,utf8_decode_1(utf8_decode_1(substr($registropago['cond_venta'],$i*15,15)))) ;
		...
		elseif (!empty($registropago['tipo'])){
			...
			for ($i=0; $i<=($total_string/15);$i++){
				$pdf->Cell(15,5,utf8_decode_1(utf8_decode_1(substr($registropago['tipo'],$i*15,15))));
				$pdf->Ln();
				$pdf->Cell(165,5,"");
				}
			$pdf->Cell(15,5,utf8_decode_1(utf8_decode_1(substr($registropago['tipo'],$i*15,15)))) ;
	...
	if (!empty($registronumoc['num_oc'])){
		...
		$sqlnumoc   = "SELECT * FROM vigomaq_intranet.factura WHERE num_factura ='$factura'";
		$resnumoc    = mysql_query($sqlnumoc,$link) or die(mysql_error()); 
		$registronumoc= mysql_fetch_array($resnumoc);
	...
	while ($registrodet = mysql_fetch_array($resdet)) {
		...
		if (($registrodet['cod_repuesto'] == 0 )&&($registrodet['cod_equipo'] == 0 )) {
			...
			$pdf->SetWidths(array(30,100,20,17,20));
			$pdf->SetAligns(array('L','L','L','L','L'));
			
			$detalle = $registrodet['otros_reparacion'];
			
			$pdf->Row(array($registrodet['cantidad'],utf8_decode_1(utf8_decode_1($detalle)),"$".number_format($registrodet['valor_unitario'], 0, ",", "."),"","$".number_format($registrodet['total_rep'], 0, ",", ".")));
	...
	//16247 -> 20049
			...
			$pdf->Cell(30,6,$registrodet['dias_arriendo']);
		 	if (!empty($valor1)){
				...
				$detalle .= "Dias Arriendo de ".utf8_decode_1($registronob['nombre_equipo']);
			...
			$pdf->Cell(100,6,utf8_decode_1(utf8_decode_1((utf8_decode($detalle)))));			  
			$link=Conectarse();
			...
			$pdf->Ln();
			$pdf->Cell(30);
			$pdf->Cell(100,6,utf8_decode_1(utf8_decode_1((utf8_decode($detalle_1)))),0,0,'L');
			$pdf->Ln();
		}
	}
	...
	$pdf->Cell(100,5,utf8_decode_1((strtoupper(docenumeros($total))))) ;
	$pdf->Ln();
