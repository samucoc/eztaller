...
function RegistroGrabado() {
  ...
  document.location = 'fecha_cierre.php';
}
 //-->
</script>
...
<script type="text/javascript">
	function eliminar(celda)
	{
		...
		{
		   alert ("Eliminas");
     	   document.forms.fecha_cierre.php.value=celda;
...
function Eliminar_onClick() {      
         document.nuevo.action="fecha_cierre.php?Accion=eliminando"
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">Fecha Corte</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS Fecha Corte </span></div></td>
    </tr>
    <tr>
      <td height="16"></td>
    </tr>
    <tr>
      <td><form action="fecha_cierre.php" method="post" name="frmDatos" id="frmDatos">
          <table width="550" height="30">
            <tr>
              <td width="19%" class='mini_titulo'><div align="left"><font><strong>Fecha :</strong></font></div></td>
              <td width="37%" class='mini_titulo'><font>
                ...
                <button type="text" id="cal-button-1">...</button>
                <script type="text/javascript">
			            Calendar.setup({
        ...
        <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" class="sortable" id="unique_id" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th width="10%" bgcolor="#06327D" class="CONT"></th>
              <th width="49%" bgcolor="#06327D" class="CONT"><span class="Estilo17">Fecha</span></th>
              <th width="13%" bgcolor="#06327D" class="CONT">
              <span class="Estilo17 Estilo13 Estilo15">Eliminar</span></th>
            </tr>
            <?php
			$sql="SELECT c_ncorr, date_format(fecha, '%d-%m-%Y') as fecha FROM cierres order by c_ncorr";
		 ...
		 ?>
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
             <td bgcolor="#fff" style="color:#fff; padding: 5px;">
			  <?php 
			  	   echo '' . $registro['c_ncorr'] . ''	
			  ?> </td>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM cierres WHERE c_ncorr =".$_POST['txt_cod'];
				...
				if (empty($_POST[txt_fecha])){  
					$link=mensaje();
					   ...
					   $codigo = (string)(int)$codigo;
						if (empty($codigo)){
						   mysql_query("insert into cierres (fecha) value ('$fecha')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";						
					  } else {
							if (($_POST['txt_fecha'])&&($fecha!='0000-00-00')&&($fecha!='')){
								echo $sql = "UPDATE cierres SET fecha='$fecha' where c_ncorr='$codigo'";
								$res  = mysql_query($sql) or die(mysql_error());
								$okas="1";
								echo "<script type='text/javascript'>RegistroGrabado();</script>";
								$txt_cod="";
...
<br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=fecha_cierre.php'>";?>
</div>
