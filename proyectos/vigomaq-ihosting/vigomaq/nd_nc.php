...
<head>
  <title>Sistema de Arriendo y ncci&oacute;n - Vigomaq</title>
...
<table width="98%" border="0">
   <tr>
     ...
     <td width="48%" valign="middle"><div align="right" class="Estilo2 Estilo25"><br />
       <br />
       <br />
       <span class="Estilo23">Sistema de Arriendo y ncción - Vigomaq</span></div></td>
   </tr>
</table>
  <div id="div-menu">
    <?php 
...
<table width="100%" border="0">
  <tr>
   ...
   ?>
            <?php
      {
        $num_nc =  $_POST["txt_nota_credito"];
        if (empty($_POST["txt_nota_credito"])) $num_nc =  $_GET["txt_nota_credito"];
        ...
        if (!empty($num_nc))
        {
          ...
          $resnc         = mysql_query($sqlnc,$link) or die(mysql_error()); 
          $registronc    = mysql_fetch_array($resnc);
          $fecha_nc     = $registronc['fecha'];
          list($d,$m,$a)  = explode('-',$fecha_nc);
          $fecha_nc = $a.'-'.$m.'-'.$d;
     ...
     ?>
      ...
      <tr>
        <td valign="top"><form action="nd_nc.php" method="post"  name="frmDatos" id="frmDatos">
          <table width="100%" border="0" align="center">
            ...
            <tr>
              ...
              <td width="1%">:</td>
       <?php 
            $sql_num_nc     = "SELECT (COALESCE(num_nota_cred  ,0)) as ncorr
                  FROM nota_debito
                  WHERE num_nota_cred  >0
                  order by num_nota_cred   desc";
          
          $res_num_nc     = mysql_query($sql_num_nc,$link) or die(mysql_error()); 
          $registro_num_nc= mysql_fetch_array($res_num_nc);
          $num_nc_nuevo = $registro_num_nc['ncorr'];

          
          if ($registro_num_nc['ncorr']=='') $num_nc_nuevo= 0;
          else $num_nc_nuevo = $registro_num_nc['ncorr']+1;


          if ($num_nc_nuevo=='') $num_nc_nuevo=1;

            $sql_filtro = "select * 
                    from folios_dte
                    where desde <= '".$num_nc_nuevo."' and 
                        hasta >= '".$num_nc_nuevo."' and 
                        tipo = 56";
         ...
         if (($_POST['txt_num_nd']=='')&&($registrofact['txt_num_nd']=='')) echo $num_nc_nuevo;
            ...
            echo $_POST['txt_num_nd'];
              ...
              <td height="8" colspan="2"><div align="left">
                <input name="cal-field-1" type="text" id="cal-field-1" value="<?php echo($_POST["cal-field-1"]) ;?>" size="10" maxlength="10"/>
            ...
            <tr>
              ...
              <td height="8" >
                <select name="accion" id="accion">
                  <option value="1">Anula Nota de Credito</option>
                </select>
              </td>
              </tr>
            <tr>
                        <tr>
              <td height="8" colspan="7" bgcolor="#06327D"><span class="Estilo24">DATOS Nota de Credito</span></td>
              </tr>
            <tr>
              <td height="8">N&deg; Nota de Credito</td>
              <td height="8">:</td>
              <td height="8"><span class="Estilo241">
                <input name="txt_nota_credito" type="text"value="<?php if (!empty($_POST["txt_nota_credito"])) {echo ($_POST["txt_nota_credito"]);}else{ echo($_GET["txt_nota_credito"]);}?>" size="10" maxlength="10" />
              ...
              <td height="8" align="right">Fecha Emision Nota de Credito</td>
              <td height="8" colspan="2"><input name="fecha" type="text" id="cal-field-2" value="<?php if (!empty($fecha_nc)) {echo ($fecha_nc);}?>" size="10" maxlength="10" disabled="disabled"/></td>
              <td>&nbsp;</td>
            </tr>
         ...
         if (!empty($registrocli['cod_comuna']))
            ...
            <tr>
              ...
              <td height="8"></td>
              <td height="8"></td>
              <td align="LEFT" style="float: right"><input type="submit" name="OK" title="Generar Nota de Debito" value="Guardar y Seguir" style="background-image:url(images/guardar.png); width:45px; height:45px;" class="formato_boton" />
    ...
    ?>
        ...
        <?php }?>
            ...
            <tr>
              ...
              <td align="center" valign="middle"><a href="#" id="agregar-fila"><img src="images/guardar.gif" alt="" class="formato_boton" style="width:46px; height:52px;" title="Agregar Repuesto a nc" /></a></td>
            </tr>
            <tr>
              <td colspan="7">
     ...
     ?>
      ...
      {   
        if (empty($_POST['txt_nota_credito']))
        {  
          $link=mensaje();
        } else {
  ...
  <?php
    function mensaje()
        {
            echo "<script>
            alert('Ingrese número de nota de Credito, fecha y/o número de nc');
            </script>";
        }
  ?>
  <?php 
  ...
  if ((empty($num_nc))or(empty($num_nd))or($nro>0)or(empty($fecha))){  
    $link=mensaje();
  } else {
    $link=Conectarse();
    ...
    if (mysql_num_rows($resf)>0) {
      ...
      $sql_001  = "insert into nota_debito (num_nota_cred,fecha,num_factura,accion) 
              values ('$num_nd','$fecha','$num_nc','$accion')";
      //echo "<br />";
