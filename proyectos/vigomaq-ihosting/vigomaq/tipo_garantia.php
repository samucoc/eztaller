...
function RegistroGrabado() {
  ...
  document.location = 'tipo_garantia.php';
}
 //-->
</script>
...
function asignar_valor(celda) {
  ...
  document.forms[0]['txt_tipo_garantia'].value = com;
}
</script>
...
<script type="text/javascript">
	function eliminar(celda)
	{
		...
		{
		   alert ("Eliminas");
     	   document.forms.tipo_garantia.php.value=celda;
...
function Eliminar_onClick() {      
         document.nuevo.action="borra_tipo_garantia.php?Accion=eliminando"
...
<table width="550" border="0" align="center">
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">TIPO DE GARANTIA</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS TIPO DE GARANTIA</span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td><form action="tipo_garantia.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
            <tr>
              <td width="138" class='mini_titulo'><div align="left"><font><strong>Tipo de Garantia:</strong></font></div></td>
              <td width="182"  class='mini_titulo'><font>
                <input type="hidden" name="txt_cod" size="20" maxlength="30" > <input type="text" name="txt_tipo_garantia" size="25" maxlength="25" />
              </font> </td>
			...
			function mensaje()
				{
					echo "<script>
					alert('Debe Ingresar Tipo de Garantia');
					</script>";
				}
		   ?>
          </table>
        <table class="sortable" id="unique_id" width="100%" border="0" align="center" cellpadding="1" cellspacing="1" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th width="38%" bgcolor="#06327D"><span class="Estilo17">C&oacute;digo Tipo de Garantia</span></th>
              <th width="49%" bgcolor="#06327D" class="CONT"><span class="Estilo17">Tipo de Garantia</span></th>
              <th width="13%" bgcolor="#06327D" class="CONT"><?php
			$sql="SELECT * FROM tipo_garantia order by cod_tipo_gar";
		 ...
		 ?>
            ...
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bgcolor="#FFFFFF">
			  <?php
				   $cantidad = strlen($registro['cod_tipo_gar']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['cod_tipo_gar'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['cod_tipo_gar'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['cod_tipo_gar'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['cod_tipo_gar'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['cod_tipo_gar'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['cod_tipo_gar'] . ' ';  }		   
	      	 	?>
			  </td>
              <td bgcolor="#FFFFFF"><?php echo '' . $registro['tipo_garantia'] . ' ';?></td>
              <td align="center" bgcolor="#FFFFFF">
              <input type="submit" name="borrar" title="Eliminar Tipo de Garantía" value="Borrar"  onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;"<?php echo $estado_objetos ;?> style="background-image:url(images/error.png); width:16px; height:16px;" class="formato_boton"/>
              
              <!--<input type="image" name="borrar" value="Borrar" title="Eliminar Tipo de Garant�a" src="images/error.png" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;"<?php echo $estado_objetos ;?> />--></td>
          </tr>
            <tr>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM tipo_garantia WHERE cod_tipo_gar =".$_POST['txt_cod'];
	 ...
	 ?>
		 ...
		 $link=Conectarse();
		  if($_POST['OK']=='Guardar y Seguir'){
				if (empty($_POST[txt_tipo_garantia])){  
					$link=mensaje();
				} else {  
					   $codigo      = $_POST['txt_cod'];
					   $tipo_garantia  = strtoupper($_POST['txt_tipo_garantia']);	
					   $codigo      = (string)(int)$codigo;
						if (empty($codigo)){
						   mysql_query("insert into vigomaq_intranet.tipo_garantia (tipo_garantia) value ('$tipo_garantia')",$link);
						   $okas    ="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod ="";						
					  } else {
							$sql  = "UPDATE vigomaq_intranet.tipo_garantia SET tipo_garantia='$tipo_garantia' where cot_tipo_gar='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas  ="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
  ...
  <br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=tipo_garantia.php'>";?>
</div>
