  ...
  <table width="80%" height="427" border="0" align="center">
    ...
    <tr>
      <td height="372" colspan="2" valign="top"><form method="post" name="frmDatos" id="frmDatos">
          <table width="100%" border="0" align="left">
            ...
            <tr class="sortable">
              ...
              <th bgcolor="#06327D"><div align="center" class="Estilo17">Precio</div></th>
			...
			$sqldet="SELECT * FROM  det_factura where num_factura = '$num_factura' order by cod_equipo ASC";
		...
		?>
            ...
            <tr bordercolor="#FFFFFF" class="sortable">
              <td align="left"><?php if (!empty($registrodet['cod_repuesto'])) {
			echo($registrodet['cantidad']);}else{echo("1");}?></td>
              <td align="left"><?php 					  
			  if (!empty($registrodet['cod_repuesto'])) {
				  $sqlnomrep="SELECT cod_repuesto, nombre_repuesto FROM repuesto where cod_repuesto =".$registrodet['cod_repuesto'];
				
				  $resnomrep = mysql_query($sqlnomrep,$link) or die(mysql_error()); 
				  $registronrep = mysql_fetch_array($resnomrep);
				  echo htmlentities($registronrep['nombre_repuesto']);
	
			  }else{

				  $sqlnomob="SELECT cod_equipo, nombre_equipo FROM equipo where cod_equipo =".$registrodet['cod_equipo'];
			 ...
			 ?></td>
              <td align="right"><?php 
			  if (!empty($registrodet['cod_equipo'])){
              ...
              <td align="right"><?php if (!empty($registrodet['cod_equipo'])){echo "$".number_format($valor, 0, ",", ".") ; $costo_tot= $costo_tot + $valor; }else{ echo("$".number_format($registrodet['total_rep'], 0, ",", ".")); $costo_tot= $costo_tot + $registrodet['total_rep'];}?></td>
              <td align="right">&nbsp;</td>
            </tr>
            <tr class="sortable">
		 ...
		 ?>
              ...
              <td height="21" align="right"><em><strong>
              <input type="image" name="impresion" value="Impresion" src="images/impresora.gif" onclick="window.print();elimina=confirm('�Impresion Correcta?');return elimina;" width="25" height="25" title="Imprimir Factura" class="oculto" />
              </strong></em></td>
            </tr>
          </table>
           <?php	
	  if ($_POST['impresion']=='Impresion')
	  {
		...
		echo('borrar');
		echo "<script language=Javascript> location.href=\"anular_fact.php?txt_factura=".$num_factura2."&fact1=".$num_factura."\"; </script>";
