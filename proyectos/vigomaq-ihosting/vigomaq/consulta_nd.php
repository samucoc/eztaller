   ...
   ?>
            <?php
      {
        ...
        if (!empty($num_factura))
        {
          ...
          $sqlfact     = "SELECT * from nota_credito where num_nota_cred ='$num_factura'";
          
          $res          = mysql_query($sqlfact,$link) or die(mysql_error()); 
          if (mysql_num_rows($res)>0){
            $flag='1';
            $num_nota_cred = $num_factura;
            $registro     = mysql_fetch_array($res);
            $fecha        = $registro['fecha'];
            $fecha_nota_credito       = $registro['fecha'];
     ...
     ?>
      ...
      <tr>
        <td valign="top"><form action="consulta_nd.php" method="post"  name="frmDatos" id="frmDatos">
          <table width="100%" height="544" border="0" align="center">
            <tr>
              <td colspan="7" bgcolor="#06327D"><span class="Estilo24">DATOS NOTA DE DEBITO</span><span class="Estilo24">
                <div align="right">
            ...
            <tr>
              ...
              <td width="18%"><span class="Estilo241">
                <input name="num_nd" type="text"value="<?php 
          if (!empty($_GET["num_nd"])){
            echo($_GET["num_nd"]);
          }else{
            echo $_POST['num_nd'];
              ...
              <td width="28%" height="8"  align="right">Fecha Emision Nota Debito</td>
            ...
            <tr>
              ...
              <td height="8" align="right">Fecha Emisión Nota Crédito</td>
              <td height="8" colspan="2"><input name="fecha" type="text" id="cal-field-2" value="<?php echo $fecha_nota_credito ?>" size="10" maxlength="10" disabled="disabled"/></td>
              <td>&nbsp;</td>
            </tr>
            <tr>
         ...
         if (!empty($registrocli['cod_comuna']))
            ...
            <tr>
              ...
              <td><a href="consulta_nd.php" class="menulink">
                <input name="Limpiar" type="image" title="Limpiar"  width="42" height="42" value="Limpiar"  src="images/clean.png"/>
                </a><a id="vista-previa-nd" href="#" download><img width="42" src="images/gest_fin/factura.png" alt="" border="0" /></a></td>
            </tr>
            <tr>
              <td></td>
              <td></td>
        ...
        $link=Conectarse();
        $sqlnc = "SELECT * FROM det_nd WHERE num_nd ='$num_nd'";
        $resnc = mysql_query($sqlnc,$link) or die(mysql_error()); 
...
<script>
  ...
  $(window).load(function() {
    $("#vista-previa-nd").attr('href','classes/nd/vista-previa-nd.php?num_nd=<?php echo $num_nd?>');
    $("#vista-previa-nd").attr('title','Descargar Nota de Débito : <?php echo $num_nd?>');
    });       
</script>
</body>
