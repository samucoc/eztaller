...
function RegistroGrabado() {
  ...
  document.location = 'sucursales.php';
}
 //-->
</script>
...
function asignar_valor(celda) {
  ...
  document.forms[0]['txt_sucursal'].value = com;
}
</script>
...
<script type="text/javascript">
	function eliminar(celda)
	{
		...
		{
		   alert ("Eliminas");
     	   document.forms.sucursales.php.value=celda;
...
function Eliminar_onClick() {      
         document.nuevo.action="borra_sucursal.php?Accion=eliminando"
...
<table width="98%" border="0">
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">SUCURSALES VIGOMAQ</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS SUCURSALES </span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td><form action="sucursales.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
            <tr>
              <td width="15%" class='mini_titulo'><div align="left"><font><strong>Sucursal:</strong></font></div></td>
              <td width="58%"  class='mini_titulo'><font>
                <input type="hidden" name="txt_cod" size="20" maxlength="30" > <input type="text" name="txt_sucursal" size="50" maxlength="50" />
              </font> </td>
          ...
          <table class="sortable" id="unique_id" border="0" align="center" cellpadding="1" cellspacing="1" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th bgcolor="#06327D"><span class="Estilo17">C&oacute;digo Sucursal </span></th>
              <th bgcolor="#06327D" class="CONT"><span class="Estilo17">Sucursal</span></th>
              <th bgcolor="#06327D" class="CONT">
              <span class="Estilo17 Estilo13 Estilo15">Eliminar</span></th>
            </tr>
            <?php
			$sql="SELECT * FROM sucursales order by cod_sucursal";
			$res = mysql_query($sql) or die(mysql_error()); 
			while ($registro = mysql_fetch_array($res)) {
		 ?>
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td><?php 
			  	   $cantidad = strlen($registro['cod_sucursal']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['cod_sucursal'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['cod_sucursal'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['cod_sucursal'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['cod_sucursal'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['cod_sucursal'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['cod_sucursal'] . ' ';  }
			  ?></td>
              <td><?php echo '' . $registro['sucursal'] . ' ';?></td>
              <td align="center">
              
              
              <input type="submit" name="borrar" title="Eliminar Sucursal" value="Borrar" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> style="background-image:url(images/error.png); width:16px; height:16px;" class="formato_boton" />
              
              
             <!--<input type="image" name="borrar" value="Borrar" title="Eliminar Sucursal" src="images/error.png" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> />--></td>
            </tr>
            <tr>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM sucursales WHERE cod_sucursal =".$_POST['txt_cod'];
	 ...
	 ?>
		 ...
		 $link=Conectarse();
		  if($_POST['OK']=='Guardar y Seguir'){
				if (empty($_POST[txt_sucursal])){  
					$link=mensaje();
				} else {  
					   $codigo       = $_POST['txt_cod'];
					   $nom_sucursal = strtoupper($_POST['txt_sucursal']);
					   $codigo=(string)(int)$codigo;
						if (empty($codigo)){
						   mysql_query("insert into sucursales (sucursal) value ('$nom_sucursal')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";						
					  } else {
							$sql = "UPDATE sucursales SET sucursal='$nom_sucursal' where cod_sucursal='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
  ...
  <br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=sucursales.php'>";?>
</div>
