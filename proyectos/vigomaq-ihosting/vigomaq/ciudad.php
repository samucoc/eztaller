...
function asignar_valor(celda) {
  ...
  document.forms[0]['txt_ciudad'].value = com;
}
</script>
...
function Eliminar_onClick() {      
         document.nuevo.action="borra_ciudad.php?Accion=eliminando"
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">CIUDADES</div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS CIUDAD </span></div></td>
    </tr>
    <tr>
      <td height="16"></td>
    </tr>
    <tr>
      <td><form action="ciudad.php" method="post" name="frmDatos" id="frmDatos">
          <table width="550" height="30">
            <tr>
              <td width="19%" class='mini_titulo'><div align="left"><font><strong>Ciudad :</strong></font></div></td>
              <td width="37%" class='mini_titulo'><font>
                <input type="hidden" name="txt_cod" size="20" maxlength="30" > <input type="text" name="txt_ciudad" size="25" maxlength="25" />
              </font> </td>
        ...
        <table width="550" border="0" align="center" cellpadding="1" cellspacing="1" class="sortable" id="unique_id" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th width="38%" bgcolor="#06327D"><span class="Estilo17">C&oacute;digo Ciudad </span></th>
              <th width="49%" bgcolor="#06327D" class="CONT"><span class="Estilo17">Ciudad</span></th>
              <th width="13%" bgcolor="#06327D" class="CONT">
              <span class="Estilo17 Estilo13 Estilo15">Eliminar</span></th>
            </tr>
            <?php
			$sql="SELECT * FROM ciudad order by cod_ciudad";
		 ...
		 ?>
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bgcolor="#FFFFFF">
			  <?php 
			  	   $cantidad = strlen($registro['cod_ciudad']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['cod_ciudad'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['cod_ciudad'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['cod_ciudad'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['cod_ciudad'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['cod_ciudad'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['cod_ciudad'] . ' ';  }		
			  ?> </td>
              <td bgcolor="#FFFFFF"><?php echo '' . $registro['ciudad'] . ' ';?></td>
              <td align="center" bgcolor="#FFFFFF">
              
                <input type="submit" name="borrar" value="Borrar" title="Eliminar Ciudad" src="images/error.png" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> style="background-image:url(images/error.png); height:16px; width:16px;" class="formato_boton" />
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM ciudad WHERE cod_ciudad =".$_POST['txt_cod'];
				...
				if (empty($_POST[txt_ciudad])){  
					$link=mensaje();
					   ...
					   $codigo = (string)(int)$codigo;
						if (empty($codigo)){
						   mysql_query("insert into ciudad (ciudad) value ('$ciudad')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";						
					  } else {
							$sql = "UPDATE ciudad SET ciudad='$ciudad' where cod_ciudad='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
...
<br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=ciudad.php'>";?>
</div>
