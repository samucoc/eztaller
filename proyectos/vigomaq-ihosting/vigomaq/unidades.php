...
function RegistroGrabado() {
  ...
  document.location = 'unidades.php';
}
 //-->
</script>
...
function asignar_valor(celda) {
  ...
  document.forms[0]['txt_unidad'].value = com;
}
</script>
...
<script type="text/javascript">
	function eliminar(celda)
	{
		...
		{
		   alert ("Eliminas");
     	   document.forms.unidades.php.value=celda;
...
function Eliminar_onClick() {      
         document.nuevo.action="borra_unidad.php?Accion=eliminando"
...
<table width="98%" border="0">
   <tr>
	...
	<div id="div-menu">
		...
		?>
	</div><table width="548" height="219" border="0" align="center">
    <tr>
      <td>      <?php
			{
			include("conex.php");
    ...
    <tr>
      <td height="31"><div align="center" class="Estilo6 Estilo1 Estilo8 Estilo9  Estilo12">
          <div align="right" class="Estilo19">
            <div align="right" class="Estilo20">UNIDADES DE MEDIDA </div>
          </div>
      </div></td>
    </tr>
    <tr>
      <td height="16" valign="top" bgcolor="#06327D"><div align="left"><span class="Estilo7">DATOS UNIDADES </span></div></td>
    </tr>
    <tr>
      <td width="664" height="16"></td>
    </tr>
    <tr>
      <td height="121"><form action="unidades.php" method="post" name="frmDatos" id="frmDatos">
          <table width="100%" height="30">
            <tr>
              <td width="138" class='mini_titulo'><div align="left"><font><strong>Unidad :</strong></font></div></td>
              <td width="277"  class='mini_titulo'><font>
                <input type="hidden" name="txt_cod" size="20" maxlength="30" > <input type="text" name="txt_unidad" size="40" maxlength="40" />
              </font> </td>
			...
			function mensaje()
				{
					echo "<script>
					alert('Debe Ingresar Unidad de Medida');
					</script>";
				}
		   ?>
          </table>
          <table class="sortable" id="unique_id" width="100%" border="0" align="center" cellpadding="1" cellspacing="1" >
            <tr title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <th width="38%" bgcolor="#06327D"><span class="Estilo17">C&oacute;digo Unidad </span></th>
              <th width="49%" bgcolor="#06327D" class="CONT"><span class="Estilo17">Unidad de Medida </span></th>
              <th width="13%" bgcolor="#06327D" class="CONT"><?php
			$sql="SELECT * FROM unidad order by cod_unidad";
		 ...
		 ?>
            ...
            <tr bordercolor="#FFFFFF" bgcolor="#B4B4B4" title="Clic para mostrar contenido" class="CONT" onmouseover="this.style.cursor='hand';this.style.cursor='pointer';" onclick="asignar_valor(this)">
              <td bordercolor="#999999" bgcolor="#FFFFFF"><?php 
			  	   $cantidad = strlen($registro['cod_unidad']); 
				   if ($cantidad==1) { echo ("00000" .('' . $registro['cod_unidad'] . ' ') );  }
				   if ($cantidad==2) { echo ("0000" .('' . $registro['cod_unidad'] . ' ') );  }
				   if ($cantidad==3) { echo ("000" .('' . $registro['cod_unidad'] . ' ') );  }
				   if ($cantidad==4) { echo ("00" .('' . $registro['cod_unidad'] . ' ') );  }
				   if ($cantidad==5) { echo ("0" .('' . $registro['cod_unidad'] . ' ') );  }
				   if ($cantidad==6) { echo '' . $registro['cod_unidad'] . ' ';  }		
			  ?>              </td>
              <td bordercolor="#999999" bgcolor="#FFFFFF"><?php echo '' . $registro['unidad'] . ' ';?></td>
              <td align="center" bordercolor="#999999" bgcolor="#FFFFFF">
              <input type="submit" name="borrar" title="Eliminar Unidad de Medida" value="Borrar" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> style="background-image:url(images/error.png); width:16px; height:16px;" class="formato_boton"/>
              
              <!--<input type="image" name="borrar" value="Borrar" title="Eliminar Unidad de Medida" src="images/error.png" onclick="elimina=confirm('&iquest;Esta seguro de que quiere Eliminar?');return elimina;" <?php echo $estado_objetos ;?> />--></td>
            </tr>
            <tr>
		...
		if($_POST['borrar']=='Borrar')
		 { 
			 $link=Conectarse();
			 $sql = "DELETE FROM unidad WHERE cod_unidad =".$_POST['txt_cod'];
				...
				if (empty($_POST[txt_unidad])){  
					$link=mensaje();
					   ...
					   $codigo = (string)(int)$codigo;
						if (empty($codigo)){
						   mysql_query("insert into vigomaq_intranet.unidad (unidad) value ('$unidad')",$link);
						   $okas="1";
						   echo "<script type='text/javascript'>RegistroGrabado();</script>";
						   $txt_cod="";						
					  } else {
							$sql = "UPDATE vigomaq_intranet.unidad SET unidad='$unidad' where cod_unidad='$codigo'";
							$res  = mysql_query($sql) or die(mysql_error());
							$okas="1";
							echo "<script type='text/javascript'>RegistroGrabado();</script>";
							$txt_cod="";
  ...
  <br />
  <?php if ($valor1!=1) echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=unidades.php'>";?>
</div>
