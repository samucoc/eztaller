...
include("../conex.php");
	...
	class PDF extends FPDF{
		...
		$pdf->SetFont('Arial','',9);
			...
			$row_equipos  = mysql_fetch_array($res_equipos);
			$linea= $row_equipos['fila_num_gd'];
				...
				for ($i=0;$i<10;$i++){
					...
					if (!empty($row_bus_equ_cam['num_gd_salida'])){
						...
						$row_nom_equ = mysql_fetch_array($res_nom_equ);
						$nombre_equipo = $row_nom_equ['nombre_equipo'];
						$pdf->Cell(20,5,utf8_decode_1(substr($nombre_equipo,-7,7)),1,0,'C',0); 
						}
					else{
								...
								FROM equipos_arriendo
									inner join factura 
										on factura.cod_arriendo = equipos_arriendo.cod_arriendo 
											and equipos_arriendo.arrendado_hasta = factura.fecha
									...
									where equipos_arriendo.cod_arriendo = ".$cod_arriendo." 
										...
										and factura.num_factura in ('".$string_num_factura."')
				...
				$pdf->Cell(50,5,utf8_decode_1("Inicio Arriendo               ".$precio."           x ".$row_inicial_1['dias_arriendo']));
				$pdf->SetXY(150,122);
				...
				$pdf->Cell(50,5,utf8_decode_1("Vence : ".$arrendado_hasta));
				$pdf->SetXY(65,132);
				...
				$pdf->Ln();
				$pdf->SetXY(15,142);
				$pdf->Cell(50,5,utf8_decode_1("Observaciones : Dias Ajuste = ".$row_inicial_1['dias_ajuste']));
				$pdf->Ln();
				...
				for($i=0; $i<3 ; $i++){				
					...
					$precio_equipo = $row_inicial_1['tot_arriendo']/$row_inicial_1['dias_arriendo'];
					$pdf->Cell(50,5,utf8_decode_1("Renovacion o Saldo por          ".$row_inicial_1['dias_arriendo']."                dias a ".$precio));
