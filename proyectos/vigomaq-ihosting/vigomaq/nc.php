  ...
  <tr>
   ...
   ?>
     ...
     ?>
      ...
      <tr>
        <td valign="top"><form action="nc.php" method="post"  name="frmDatos" id="frmDatos">
          <table width="100%" height="402" border="0" align="center">
            <tr>
              <td height="20" colspan="7" bgcolor="#06327D"><span class="Estilo24">DATOS NOTA DE CREDITO</span><span class="Estilo24">
 ...
 <?php 
          ...
          if ($num_factura_nuevo=='') $num_factura_nuevo=1;
            ...
            if (mysql_num_rows($res_filtro)==0){
              $num_factura_nuevo = "Sin Folio";
              }
          
            ?>              <td width="28%"><span class="Estilo241">
                <input name="txt_num_nc"  id="txt_num_nc" type="text"value="<?php 
          if (($_POST['txt_num_nc']=='')&&($registrofact['txt_num_nc']=='')) echo $num_factura_nuevo;
              ...
              <td height="8" >
                <select name="accion" id="accion">
                  <?php 
                      $accion = $_POST['accion'];
                      if ($accion =='1'){
                        echo "<option value='1'>Anula Documento de Referencia</option>";
                        }
                      else if ($accion =='2'){
                        echo "<option value='2'>Corrige Texto Documento de Referencia</option>";
                        }
                      else if ($accion =='3'){
                        echo "<option value='3'>Corrige montos</option>";
                        }
         ...
         if (!empty($registrocli['cod_comuna']))
            ...
            <tr>
              ...
              <td><input type="submit" name="OK" title="Generar Nota de Credito" value="Guardar y Seguir" style="background-image:url(images/guardar.png); width:45px; height:45px;" class="formato_boton" />
  ...
  if (empty($num_factura)or(empty($num_nc)or(empty($fecha)))){  
    $link=mensaje();
  } else {
    $link=Conectarse();
    ...
    if (mysql_num_rows($resf)>0) {
      ...
      $sql_001  = "insert into nota_credito (num_nota_cred,cod_cliente,fecha,num_factura, accion) 
              values ('$num_nc','$cod_cliente','$fecha','$num_factura','$accion')";
      //echo "<br />";
