...
$xajax->setRequestURI("sg_oc_ingreso.php");
...
function CargaPagina($data){
    ...
    $sql_busca = "select max(oc_ncorr)+1 as grupo_mod
    				from gescolcl_taller.orden_compra";
	...
	if ($_SESSION['grupo_mod']=='') $_SESSION['grupo_mod'] = '0';

	$sql_busca = "delete from gescolcl_taller.oc_detalle_repuestos_temp";
  	$res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
    
    $sql_busca = "delete from gescolcl_taller.oc_detalle_mo_temp";
    $res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
...
function RescatarCotizacion($data){
	global $conexion;
    ...
    $objResponse = new xajaxResponse('ISO-8859-1');
		...
		$sql_insert = "INSERT INTO gescolcl_taller.oc_detalle_mo_temp(`grupo_mod`, `detalle`, `cantidad`,
						...
						FROM gescolcl_taller.cotizacion_detalle_mo WHERE ot_ncorr = '".$ot_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());

	    $mod_ncorr = $data['ot_ncorr'];

		$sql_insert = "INSERT INTO `oc_detalle_repuestos_temp`( `grupo_mod`, `cod_repuesto`, `cantidad`, `precio_neto_unitario`, 
																`iva`, `precio_unitario`, `total`) 
						select '".$_SESSION['grupo_mod']."', `cod_repuesto`, `cantidad`, `precio_neto_unitario`, `iva`, `precio_unitario`, `total`
						FROM `cotizacion_detalle_repuestos` WHERE mod_ncorr = '".$mod_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());
...
function CargarMO($data){
    ...
    $sql_genera_mo  = "select * from gescolcl_taller.oc_detalle_mo_temp
    ...
    if (mysql_num_rows($res_genera_mo)>0){
		...
		$sql_genera_mo  = "select sum(total) as total_mo, sum(cantidad*iva) as iva, sum(precio_neto*cantidad) as precio_neto
							from gescolcl_taller.oc_detalle_mo_temp
	    					where grupo_mod = '".$_SESSION['grupo_mod']."' ";
	   // $res_genera_mo	= mysql_query($sql_genera_mo,$conexion) or die(mysql_error());
		...
		$sql_genera_mo  = "select sum(total) as total_rep, sum(cantidad*iva) as iva, sum(precio_neto_unitario*cantidad) as precio_neto
							from gescolcl_taller.oc_detalle_repuestos_temp
	    					where grupo_mod = '".$$_SESSION['grupo_mod']."' ";
	    $res_genera_mo	= mysql_query($sql_genera_mo,$conexion) or die(mysql_error());
	    $row_genera_mo = mysql_fetch_array($res_genera_mo);
...
function CargarRepuestos($data){
    ...
    $sql_genera_mo  = "select * from gescolcl_taller.oc_detalle_repuestos_temp
    					where grupo_mod = '".$_SESSION['grupo_mod']."' ";
    $res_genera_mo	= mysql_query($sql_genera_mo,$conexion) or die(mysql_error());
    $i=0;
    if (mysql_num_rows($res_genera_mo)>0){
...
function Grabar($data){
	...
	$ot_ncorr 			= $data['ot_ncorr'];
	$proveedor 			= $data['rut_proveedor'];
	$usuario 			= $_SESSION["alycar_usuario"];
	$fecha_dig 			= date("Y-m-d H:i:s");

	$sql_insert = "INSERT INTO gescolcl_taller.orden_compra(`fecha`, `empresa`, `proveedor`, `cotizacion`, 
															`tipo_compra`, `iva`, `usuario`, `fecha_dig`,estado) 
		VALUES ('".$fecha."','".$codigoEmpresa."','".$proveedor."','".$ot_ncorr."','1','19', '".$usuario ."','".$fecha_dig ."','PENDIENTE')";
    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());
    $id = mysql_insert_id();
    ...
    $row_busca = mysql_fetch_array($res_busca);
		...
		$sql_insert = "INSERT INTO gescolcl_taller.oc_detalle_mo(`ot_ncorr`, `detalle`, `cantidad`, `precio_neto`, `iva`, `total_unitario`, `total`, `pagado`) 
						select '".$id."', `detalle`, `cantidad`, `precio_neto`, `iva`, `total_unitario`, `total`, `pagado` 
						FROM gescolcl_taller.oc_detalle_mo_temp WHERE grupo_mod = '".$mod_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());
    ...
    $row_busca = mysql_fetch_array($res_busca);
		...
		$sql_insert = "INSERT INTO `oc_detalle_repuestos`(`mod_ncorr`, `cod_repuesto`, `cantidad`, `precio_neto_unitario`, 
															`iva`, `precio_unitario`, `total`) 
	...
	$objResponse->addScript("alert('Orden de Compra Guardada.Nro : ".$id."')");
...
function GrabarMO($data){
    ...
    $sql_insert = "INSERT INTO gescolcl_taller.oc_detalle_mo_temp(`grupo_mod`, `ot_ncorr`, `detalle`, `cantidad`, 
    														`precio_neto`, `iva`, `total_unitario`, `total`, `pagado`) 
...
function GrabarRepuesto($data){
    ...
    $sql_insert = "INSERT INTO gescolcl_taller.oc_detalle_repuestos_temp(`grupo_mod`, `cod_repuesto`, `cantidad`, 
...
function AgregarProveedor($data){
	...
	$objResponse->addScript("showPopWin('sg_mant_tablas.php?tbl=proveedor', 'Mantenedor Proveedores', 800, 600, null);");
...
$xajax->registerFunction("RescatarCotizacion");
...
$miSmarty->display('sg_oc_ingreso.tpl');
?>
