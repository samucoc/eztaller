                        ...
                        <select name="movim_bodega" id="movim_bodega" required type="text" class="form-control" placeholder="Menús">
                            <?php 
                                ...
                                while ($row=mysqli_fetch_array($query)) {
                                            ?>
                                <option value="<?php echo $row['b_ncorr']?>"><?php echo utf8_encode($row['nombre'])?></option>
                            <?php }?>
                        </select>    
                        <span class="fa fa-user form-control-feedback right" aria-hidden="true"></span>
                    </div>
                    <div class="col-md-12 col-sm-12 col-xs-12 form-group has-feedback">
                        <input type="text" name="vendedor" id="vendedor" class="form-control pull-right" >
                        <span class="fa fa-user form-control-feedback right" aria-hidden="true"></span>
                    </div>
                    <div class="col-md-12 col-sm-12 col-xs-12 form-group has-feedback">
...
<script type="text/javascript" src="js/ingreso_vendedor.js"></script>
...
<script>
    $(function () {
        $( "#save_data" ).click(function( event ) {
            ...
            $.ajax({
                    type: "POST",
                    url: "action/add_ingreso_vendedor.php",
                    data: parametros,
