...
$xajax->setRequestURI("sg_ot_ingreso_5.php");
...
function CargaPagina($data){
    ...
    $sql_busca = "select max(ot_ncorr)+1 as grupo_mod
	...
	$sql_busca = "delete from gescolcl_taller.ot_detalle_repuestos_temp";
  	$res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
    
    $sql_busca = "delete from gescolcl_taller.ot_detalle_mo_temp";
    $res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
...
function RescatarOC($data){
    ...
    $objResponse = new xajaxResponse('ISO-8859-1');
		...
		$sql_insert = "INSERT INTO gescolcl_taller.ot_detalle_mo_temp(`grupo_mod`, `detalle`, `cantidad`,
																 `precio_neto`, `iva`, `total_unitario`, `total`, `pagado`) 
						select '".$_SESSION['grupo_mod']."', `detalle`, `cantidad`, `precio_neto`, `iva`, `total_unitario`, `total`, `pagado` 
						FROM gescolcl_taller.oc_detalle_mo WHERE ot_ncorr = '".$ot_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());

	    $mod_ncorr = $data['ot_ncorr'];

		$sql_insert = "INSERT INTO `ot_detalle_repuestos_temp`( `grupo_mod`, `cod_repuesto`, `cantidad`, `precio_neto_unitario`, 
																`iva`, `precio_unitario`, `total`) 
						select '".$_SESSION['grupo_mod']."', `cod_repuesto`, `cantidad`, `precio_neto_unitario`, `iva`, `precio_unitario`, `total`
						FROM `oc_detalle_repuestos` WHERE mod_ncorr = '".$mod_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());

	    $objResponse->addScript("xajax_CargarMO(xajax.getFormValues('Form1'));");
	    $objResponse->addScript("xajax_CargarRepuestos(xajax.getFormValues('Form1'));");
...
function CargarMO($data){
    ...
    if (mysql_num_rows($res_genera_mo)>0){
		...
		$objResponse->addAssign("divmo", "innerHTML", $miSmarty->fetch('sg_oc_ingreso_mo_list.tpl'));
		...
		$sql_genera_mo  = "select sum(total) as total_mo, sum(cantidad*iva) as iva, sum(precio_neto*cantidad) as precio_neto
							from gescolcl_taller.ot_detalle_mo_temp
	    					where grupo_mod = '".$_SESSION['grupo_mod']."' ";
	    $res_genera_mo	= mysql_query($sql_genera_mo,$conexion) or die(mysql_error());
		...
		$sql_genera_mo  = "select sum(total) as total_rep, sum(cantidad*iva) as iva, sum(precio_neto_unitario*cantidad) as precio_neto
							from gescolcl_taller.ot_detalle_repuestos_temp
	    					where grupo_mod = '".$$_SESSION['grupo_mod']."' ";
	    $res_genera_mo	= mysql_query($sql_genera_mo,$conexion) or die(mysql_error());
	    $row_genera_mo = mysql_fetch_array($res_genera_mo);
...
function CargarRepuestos($data){
    ...
    $sql_genera_mo  = "select * from gescolcl_taller.ot_detalle_repuestos_temp
    					where grupo_mod = '".$_SESSION['grupo_mod']."' ";
    $res_genera_mo	= mysql_query($sql_genera_mo,$conexion) or die(mysql_error());
    $i=0;
    if (mysql_num_rows($res_genera_mo)>0){
		...
		$miSmarty->assign('arrRegistros', $arrRegistros);
		$objResponse->addAssign("divrepuestos", "innerHTML", $miSmarty->fetch('sg_oc_ingreso_repuestos_list.tpl'));
...
function Grabar($data){
	...
	$fecha_ot		= $data['fecha_ot'];
	list($dia,$mes,$anio) = explode('/',$fecha_ot);
	$fecha_ot = $anio.'-'.$mes.'-'.$dia;
	...
	$sql_insert = "INSERT INTO `orden_trabajo`(`folio`, `empe_rut`, `fecha`, `patente`, `mecanico`, `rut_trabajador`, `usuario`, `fecha_dig`) 
		VALUES ('".$folio."','".$codigoEmpresa."','".$fecha_ot."','".$patente."','".$rut_mecanico."',
				'".$rut_trabajador ."','".$usuario ."','".$fecha_dig ."')";
    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());
    $id = mysql_insert_id();

		$sql_insert = "INSERT INTO gescolcl_taller.movim( `movim_bodega`, `movim_tipo`, empe_rut, vend_ncorr
	 													`movim_fecha`, usu_id, `movim_estado`, `movim_fecha_dig`) 
		VALUES ('1','2','".$codigoEmpresa."','".$rut_trabajador."','".$fecha."','".$usuario ."','FINALIZADO','".$fecha_dig ."')";
    	$res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());
    	$id_movim = mysql_insert_id();

	$sql_busca = "select grupo_mod
					FROM `ot_detalle_mo_temp` 
					WHERE grupo_mod = '". $_SESSION['grupo_mod']."'";
    $res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
    $row_busca = mysql_fetch_array($res_busca);
		$mod_ncorr = $row_busca['grupo_mod'];

		$sql_insert = "INSERT INTO `ot_detalle_mo`(`ot_ncorr`, `detalle`, `cantidad`, `precio_neto`, `iva`, `total_unitario`, `total`, `pagado`) 
						select '".$id."', `detalle`, `cantidad`, `precio_neto`, `iva`, `total_unitario`, `total`, `pagado` 
						FROM `ot_detalle_mo_temp` WHERE grupo_mod = '".$mod_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());
	...
	$sql_busca = "select grupo_mod
					FROM `ot_detalle_repuestos_temp` 
					WHERE grupo_mod = '". $_SESSION['grupo_mod']."'";
    $res_busca = mysql_query($sql_busca,$conexion) or die(mysql_error());
    $row_busca = mysql_fetch_array($res_busca);
		$mod_ncorr = $row_busca['grupo_mod'];

		$sql_insert = "INSERT INTO `ot_detalle_repuestos`(`mod_ncorr`, `cod_repuesto`, `cantidad`, `precio_neto_unitario`, `iva`, `precio_unitario`, `total`) 
						select '".$id."', `cod_repuesto`, `cantidad`, `precio_neto_unitario`, `iva`, `precio_unitario`, `total`
						FROM `ot_detalle_repuestos_temp` WHERE grupo_mod = '".$mod_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());

		$sql_insert = "INSERT INTO `movim_detalle`( `movim_ncorr`, `mdet_codigo`, `mdet_valor`, `mdet_cantidad`, `mdet_subneto`, `mdet_subtotal`) 
						select '".$id_movim."', `cod_repuesto`, `precio_neto_unitario`, `cantidad`, `precio_unitario`, `total`
						FROM `oc_detalle_repuestos_temp` WHERE grupo_mod = '".$mod_ncorr."'";
	    $res_insert = mysql_query($sql_insert,$conexion) or die(mysql_error());

	$objResponse->addScript("alert('Orden de Trabajo guardada.Nro : ".$id."')");
...
function GrabarMO($data){
    ...
    $sql_insert = "INSERT INTO gescolcl_taller.ot_detalle_mo_temp(`grupo_mod`, `ot_ncorr`, `detalle`, `cantidad`, 
    														`precio_neto`, `iva`, `total_unitario`, `total`, `pagado`) 
	...
	$miSmarty->assign('arrRegistros', $arrRegistros);
	$objResponse->addAssign("divmo", "innerHTML", $miSmarty->fetch('sg_ot_ingreso_mo_list.tpl'));
...
function GrabarRepuesto($data){
    ...
    $sql_insert = "INSERT INTO gescolcl_taller.ot_detalle_repuestos_temp(`grupo_mod`, `cod_repuesto`, `cantidad`, 
	...
	$miSmarty->assign('arrRegistros', $arrRegistros);
	$objResponse->addAssign("divrepuestos", "innerHTML", $miSmarty->fetch('sg_ot_ingreso_repuestos_list.tpl'));
...
$xajax->registerFunction("RescatarOC");
$xajax->registerFunction("CargarMO");
$xajax->registerFunction("CargarRepuestos");
...
$miSmarty->display('sg_ot_ingreso_2.tpl');
?>
