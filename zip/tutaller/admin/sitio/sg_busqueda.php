...
function Buscar($data, $tabla, $campo1, $campo2){
	...
	if ($v_buscar_por == '01'){
		...
		if (($v_entidad == 4) OR ($v_entidad == 7) OR ($v_entidad == 'articulo')){
			$sql = "select ta_ncorr, concat(ta_busqueda, ' ', ta_descripcion) as descripcion,  nombre
					from sgcopec.tallasnew 
						inner join sgcopec.marcas
							on tallasnew.ta_marca = marcas.marca_ncorr
					where concat(ta_busqueda, '', ta_descripcion) like '%".$v_texto."%'";
		}
	...
	while ($line = mysql_fetch_array($res)) {
		...
		$marca = str_replace('"', '', $line[2]);

		//$desc = utf8_decode($line[1]);
		
		array_push($arrRegistros, array("codigo" => $line[0], "descripcion" => $desc, "marca" => $marca));
	}
...
function TraeValor($data, $campo1, $campo2, $campo3) {
	$objResponse = new xajaxResponse('ISO-8859-1');
    
	$obj1 	= 	$data["txtObj1"];
	$obj2 	= 	$data["txtObj2"];
	$obj3 	= 	'OBLItxtMarcaProducto';
	
	$objResponse->addScript("window.parent.xajax_MuestraRegistro(xajax.getFormValues('Form1'), '$campo1', '$campo2', '$campo3', '$obj1', '$obj2', '$obj3')");
