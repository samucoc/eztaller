...
if (!class_exists('AjaxAdmin'))
{
	class AjaxAdmin extends DbExt
	{
		...
		{			
			...
			if (empty($this->data['id'])){					
				...
				if ( $t=Yii::app()->functions->validateUsername($this->data['username']) ){				
				 ...
				 if ( Yii::app()->functions->validateCompaniesEmail($this->data['contact_email'],$this->data['id']) ){
				     $this->msg=Yii::t("default","Merchant Email address is already been taken");
				      return ;
			     }	    
			...
			if (!empty($this->data['company_slug'])){
				$params['company_slug']=FunctionsV3::verifyCompaniesSlug(
				  Yii::app()->functions->seo_friendly_url($this->data['company_slug']),
				  $this->data['id']
				);
			} else {	
			    $params['company_slug']=Yii::app()->functions->createSlug($this->data['company_name']);
			}
			$params['company_rut']=addslashes($this->data['company_rut']);
			$params['company_name']=addslashes($this->data['company_name']);
			$params['company_phone']=addslashes($this->data['company_phone']);
			$params['contact_name']=addslashes($this->data['contact_name']);
			$params['contact_phone']=$this->data['contact_phone'];
			$params['contact_email']=$this->data['contact_email'];
			$params['country_code']=$this->data['country_code'];
		    ...
		    $params['commision_type']=isset($this->data['commision_type'])?$this->data['commision_type']:'';		    
		    $params['is_ready']=isset($this->data['is_ready'])?$this->data['is_ready']:1;
		    $params['time_delivery']=isset($this->data['time_delivery'])?$this->data['time_delivery']:'';
		    $params['time_close']=isset($this->data['time_close'])?$this->data['time_close']:'';
		    $params['form_payment']=isset($this->data['form_payment'])?$this->data['form_payment']:'';
		    $params['tip']=isset($this->data['tip'])?$this->data['tip']:'';
		    $params['time_delivery']=isset($this->data['time_delivery'])?$this->data['time_delivery']:'';
		    $params['time_close']=isset($this->data['time_close'])?$this->data['time_close']:'';
		    
		    if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{company}}",$params)){
		    		$this->details=Yii::app()->db->getLastInsertID();
		    	...
		    	unset($params['date_created']);
				$params['date_modified']=date('c');		
				$params['company_slug']=FunctionsV3::verifyMerchantSlug($params['company_slug'],$this->data['id']);
						
				$res = $this->updateData('{{company}}' , $params ,'id',$this->data['id']);
				if ($res){
					$this->code=1;
	                $this->msg=Yii::t("default",'Company updated.');  
		...
		public function addCompaniesProfile()
		{			
			if (empty($this->data['id'])){				
				if ( empty($this->data['username']) && empty($this->data['password'])){
					$this->msg=Yii::t("default","username & password is required");
		...
		public function companyManagerLogin()
		{			
			
            /** check if admin has enabled the google captcha*/    	    	
	    	if ( getOptionA('captcha_company_manager_login')==2){
	    		if ( GoogleCaptcha::checkCredentials()){
	    			if ( !GoogleCaptcha::validateCaptcha()){
	    				$this->msg=GoogleCaptcha::$message;
	    				return false;
			...
			if ( $res=$this->rst($stmt)){	
				if ($res[0]['status']=="active" || $res[0]['status']=="expired"){
					...
					$_SESSION['kr_company_manager_user']=json_encode($res);
					...
					$this->updateData("{{company}}",$params,'id',$res[0]['id']);
					 
					$_SESSION['kr_company_manager_user_session']=$session_token;
					$_SESSION['kr_company_manager_user_type']='admin';
				...
				$this->companyManagerUserLogin();
			}		
		}
		
		public function companyManagerUserLogin()
		{
		  	$stmt="SELECT a.*,
		          (
		            select company_name
			...
			if ( $res=$this->rst($stmt)){	
				...
				if ($res[0]['status']=="active" || $res[0]['status']=="expired"){
					...
					$_SESSION['kr_company_manager_user']=json_encode($res);
					$_SESSION['kr_company_manager_user_session']=$session_token;
					$_SESSION['kr_company_manager_user_type']='company_name';
		...
		public function companyadminLogin()
		{			
			...
			if ( $res=$this->rst($stmt)){	
				...
				//$this->msg=Yii::t("default","Either username or password is invalid.");
				$this->companyadminUserLogin();
			}		
		}
		
		public function companyadminUserLogin()
		{
		  	$stmt="SELECT a.*,
		          (
		            select company_name
    ...
    if (is_numeric($mt_delivery_miles)){    	
	    		...
	    		if ( Yii::app()->functions->code==1){
					...
					{	    				
	    				$params = array(
						  ...
						  'last_order_debt' => $raw["total"]["last_order_debt"]
	    				);	    			
	    		        ...
	    		        if(isset($_SESSION['order_id'])){
	    		        	///cambiar de insert a update!!!
		    				if ( $this->updateData("{{order}}",$params,'order_id',$_SESSION['order_id'])){
			    		...
			    		// 			$list= Yii::app()->db
									...
									// 					  from pr_order_delivery_address
									...
									// 	$id = $item['id'];
									// }		    			
				    				$this->updateData("{{order_delivery_address}}",$params_address,'order_id',$order_id);
	    ...
	    public function viewReceipt()
	    {
	    	if (isset($this->data['backend'])){
	    		...
	    		$this->updateData("{{order}}",$params,'order_id',$this->data['orderId']);
	    	}	    	    	
	    ...
	    {	    	
	    	if (isset($this->data['order_id'])){
	    		$_SESSION['order_id'] = $this->data['order_id'];
	    ...
	    public function updateConfirmOrder(){
	    	...
	    	if (isset($this->data['order_id'])){
				$list= Yii::app()->db
								 ->createCommand('update  pr_order  set 
								 				  dd_conf = "'.$this->data['text'].'"
												  where order_id = "'.$this->data['order_id'].'"')
								 ->query();
				if ($list){
					$this->code=1;
		            $this->msg='Order updated.';  
		            $this->details='Order updated.'; 
		     	} else {
					$this->msg="ERROR: Cannot update";	
		            $this->details='Client updated.';  
			 	}	
	    	} else {
	    ...
	    {	    		  
			...
			(select group_concat(addon) from {{order_details}} where order_id=a.order_id) as addon,
	    	(
	    	select group_concat(qty, ' ',item_name)
	    	from
	    	{{order_details}}
	    	where
	    	order_id=a.order_id
	    	...
	    	if ($res=$DbExt->rst($stmt)){
	    		//ar_dump($res);
	    		foreach ($res as $val) {	
					$items = $this->setFormatItem($this->getItems($val['order_id']));
	    			...
	    			$total = $val['voucher_amount']+$val['sub_total'];
	    			/*$date=prettyDate($val['date_created'],true);
	    			$date=Yii::app()->functions->translateDate($date);*/
	    			$date=FormatDateTime($val['date_created']);
	    			$list= Yii::app()->db
										->createCommand('select company_id
														  from pr_client 
					...
					$time_delivery = '';
					if($company_id>0){
					//si cliente es empresa -> hora empresa
		    			$list= Yii::app()->db
											->createCommand('select time_delivery
															  from pr_company
															  where id = "'.$company_id.'" ')
											->queryAll();
						$company_id = '';
	    			...
	    			$feed_data['aaData'][]=array(
					  ...
					  isset($val['company_2']) ? $val['company_2'] : $val['company_1'],
					  $items,
	    			  ...
	    			  prettyFormat($total, $merchant_id),
	    			  t($val['status']),
	    			  $date,
	    			  $time_delivery,
	    			  $val['dd_conf'] != '' ? 'Confirmado' : $val['dd_conf'],
	    			  $val['trans_type'] == 'delivery' ? 'Confirmado' : "<a data-id=\"".$val['order_id']."\" class=\"autoconfirm-order\" href=\"javascript:\">".Yii::t("default","AutoConfirm Orden")."</a>",
	    			  $action
	    		    );
	    		}
	    		$this->otableOutput($feed_data);
	    ...
	    {	    		    	
	    	...
	    	<div class="view-receipt-pop">
	    	 ...
	    	 <?php if ( $res=Yii::app()->functions->getOrderInfo($this->data['orderId']) ):?>
	    	    ...
	    	    <?php echo CHtml::hiddenField('order_id',$this->data['orderId'])?>
	    ...
	    {	    		    
	    	...
	    	$stmt = "SELECT a.*,
					    ...
					    (select custom_field1 					 from {{client}} 	    where client_id = a.client_id) as company,
					    (select concat(contact_phone) 			 from {{client}} 	    where client_id = a.client_id) as contact_phone
	    	
	    			FROM {{order}} a WHERE merchant_id = '$merchant_id'
			...
			if ($res = $DbExt->rst($stmt))
			{
				...
				{	    			
	    			...
	    			$total = $val['sub_total'] + $val['voucher_amount'];
	    			$feed_data['aaData'][] = array(
	    			  $val['order_id'],
		...
		{			
			...
			Yii::app()->functions->updateOptionAdmin("global_admin_sender_name",
	    	isset($this->data['global_admin_sender_name'])?$this->data['global_admin_sender_name']:'' );
...
$last_login=$val['last_login']=="0000-00-00 00:00:00"?"":date('M d,Y G:i:s',strtotime($val['last_login']));  */
		...
		{			
			...
			if ( !Yii::app()->functions->isClientLogin())
				return $this->msg = t("You must be login to use the voucher");
			
			$_SESSION['voucher_code']='';			
			
			if (!isset($this->data['voucher_code']))
				return $this->msg=Yii::t("default","Missing parameters");
			
			if ($res = Yii::app()->functions->getVoucherCodeNew($this->data['voucher_code'], $this->data['merchant_id'])){
				$res['voucher_code'] = $res['voucher_name'];
				
				// Check if the voucher can used be once and is already used
				if ($res['used_once'] == 2 && $res['number_used'] > 0)
					return $this->msg=t("Sorry this voucher code has already been used");
				
				// Validate if the coupon expire by date
				if (!empty($res['expiration'])){		
					...
					if (is_object($date_diff) && $date_diff->invert == 1 && $date_diff->d > 0)
						return $this->msg=t("Voucher code has expired");
				}
									
				// Check if the client already used this voucher
				if ( $res['found'] <= 0){
					$this->code = 1;
					$this->msg = "OK";
				...
				if ($res = Yii::app()->functions->getVoucherCodeAdmin($this->data['voucher_code'])){
					...
					// Validate if the coupon can be used on this merchant
					if (!empty($res['joining_merchant'])){	
						...
						if (!in_array($this->data['merchant_id'], (array)$joining_merchant))							
							return $this->msg = t("Sorry this voucher code cannot be used on this merchant");
					} else {
		...
		public function rptIncomingOrders()
		{
			...
			if (isset($this->data['start_date']) && isset($this->data['end_date']) &&
			   !empty($this->data['start_date']) && !empty($this->data['end_date']))
			{
				$and = " AND date_created BETWEEN  '" . $this->data['start_date'] . " 00:00:00'
												AND '" . $this->data['end_date']   . " 23:59:00'";
			}
			...
			if (isset($this->data['stats_id']) &&
				is_array($this->data['stats_id']) && count($this->data['stats_id']) >= 1)
			{
				foreach ($this->data['stats_id'] as $stats_id)
					$order_status_id .= "'$stats_id',";
			...
			if ($orders = $db->rst($query))
			{
				...
				foreach ($orders as $order)
				{
					...
					$action = "<a data-id=\"" . $order['order_id'] . "\" class=\"edit-order\" href=\"javascript:\">" . Yii::t("default", "Edit") . "</a>";
					$action = "<br/><a data-id=\"" . $order['order_id'] . "\" class=\"view-receipt\" href=\"javascript:\">" . Yii::t("default", "View") . "</a>";
					...
					$sub_total = $order['sub_total'] + $order['voucher_amount'];
					$total_client = $order['sub_total'] + $order['voucher_amount'] + $order['cart_tip_value'];
					
					$items = $this->setFormatItem($this->getItems($order['order_id']));

					$feed_data['aaData'][] = array(
						$order['order_id'],
						ucwords($order['merchant_name']),
						ucwords($order['client_name']),
						$items,
						  //strtoupper(Yii::t("default",$order['payment_type'])),
						FunctionsV3::prettyPaymentType('payment_order', $order['payment_type'], $order['order_id']),
						prettyFormat($order['voucher_amount'], $merchant_id),
						prettyFormat($sub_total, $merchant_id),
						prettyFormat($order['cart_tip_value'], $merchant_id),
						prettyFormat($total_client, $merchant_id),
						prettyFormat($order['last_order_debt'], $merchant_id),
		...
		public function rptAdminSalesRpt()
		{
	    	...
	    	$stmt="SELECT a.*,
	    	(select concat(first_name,' ',last_name) from {{client}} where 	client_id=a.client_id ) as client_name,
	    	(select contact_phone from {{client}} where client_id=a.client_id) as contact_phone,
	    	(select custom_field1 from {{client}} where client_id=a.client_id)as company    	
	    	FROM
	    	{{order}} a
	    	WHERE
	    	status NOT IN ('".initialStatus()."') $and
	    	ORDER BY order_id DESC
	    	LIMIT 0,2000
	    	";
	    	/*dump($this->data);
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		foreach ($res as $val) {
	    			...
	    			$date=Yii::app()->functions->translateDate($date);*/
					$date=FormatDateTime($val['date_created']);
					$total_income =  $val['sub_total'] + $val['voucher_amount'];
					$total_user =  ($total_income -  $val['voucher_amount']) + $val['cart_tip_value'];
	    			$feed_data['aaData'][]=array(
	    			  $val['order_id'],
					  ucwords($val['client_name']),
					  $val['company'],
					  ...
					  prettyFormat($total_user, $merchant_id),
					  prettyFormat($val['last_order_debt'], $merchant_id),
					  prettyFormat($total_income, $merchant_id),
					  ucwords($val['status']),
	    			  $date
	    			  //$action
	    		    );
...
if (!Yii::app()->functions->getMerchantOffers($merchant_id,$this->data['valid_from']." 00:00:00",$this->data['valid_to']." 00:00:00")){
		...
		public function getItems($order_id)
		{
			...
			$addons_array=[];
			$DbExt=new DbExt;
			$addon_sql="SELECT item_name, qty, addon, ingredients 
						FROM {{order_details}} 
						WHERE order_id=$order_id";
	    	if ($res=$DbExt->rst($addon_sql)){
				foreach ($res as $val)
				{
					$addons_array=[];
					$addons = json_decode($val['addon']);
		...
		public function setFormatItem($items)
		{	
			if($items!=false)
			{
				$html_items="";
				foreach ($items as $item)
				{
					...
					if(count($item['addons'])>0)
					{
						...
						{
							$addons.="<li><b>".$addon['category'].":</b> ".$addon['name']."</li>";
						}
	
						$html_addons="<ul>".$addons."</ul>";
					}
					$html_items.='<li><b>'.$item['qty'].'</b> '.$item['name'].$html_addons.'</li>';
