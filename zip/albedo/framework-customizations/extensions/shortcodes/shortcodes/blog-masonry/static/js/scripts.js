(function($){
	...
	function wplab_albedo_blog_masonry_carousel_init() {
		$('.shortcode-blog-masonry .post-gallery-carousel').each( function() {
	...
	function wplab_albedo_blog_masonry_init( gridId ) {
		...
		wplab_albedo_blog_masonry_carousel_init();
	}

	function wplab_albedo_blog_masonry_load_more( $moreElement, $shortcodeContainer, $grid, nextPage, newPage, maxPages ) {
		...
		$.ajax({
			...
			success: function( answer ) {
				...
				wplab_albedo_blog_masonry_init( $grid.attr('id') );
	...
	window.albedo_blog_masonry_init = function() {
		$('.shortcode-blog-masonry').each( function() {

			var $shortcode = $(this),
			gridId = $shortcode.find('ul.grid').attr('id'),
			$grid = $('#' + gridId);

			wplab_albedo_blog_masonry_init( gridId );
			...
			$shortcode.find( '.ajax-pagination-button').click( function() {
				...
				wplab_albedo_blog_masonry_load_more( $link, $shortcode, $grid, nextPage, newPage, maxPages );
					...
					finished: function() {
						...
						$(window).scroll( function(){
							...
							if( $(window).scrollTop() >= pos && $infiniteScrollPagination.hasClass('loading') == false && ( nextPage <= maxPages ) ) {
								...
								wplab_albedo_blog_masonry_load_more( $infiniteScrollPagination, $shortcode, $grid, nextPage, newPage, maxPages );
	...
	window.albedo_blog_masonry_init();
