...
class FW_Shortcode_Google_Map extends FW_Shortcode {
		...
		if (empty($this->data)) {
			$this->data = apply_filters('fw_shortcode_map_provider', array(
				'custom' => array(
					...
					'label'      => __('Custom','albedo'),
					'options'    => array(
						'locations' => array(
							'label' => __('Locations', 'albedo'),
							'popup-title' => __('Add/Edit Location', 'albedo'),
							'type' => 'addable-popup',
							'desc' => false,
							'template' => '{{  if (location.location !== "") {  print(location.location)} else { print("' . __('Note: Please set location', 'albedo') . '")} }}',
							'popup-options' => array(
								'location' => array(
									'type' => 'map',
									'label' =>__('Location','albedo'),
								),
								'title' => array(
									'type' => 'text',
									'label' => __('Location Title', 'albedo'),
									'desc' => __('Set location title', 'albedo'),
								),
								'description' => array(
									'type'  => 'textarea',
									'label' => __('Location Description', 'albedo'),
									'desc'  => __('Set location description', 'albedo')
								),
								'url' => array(
									'type'  => 'text',
									'label' => __('Location Url', 'albedo'),
									'desc'  => __('Set page url (Ex: http://example.com)', 'albedo'),
								),
								'icon' => array(
									'label' => __('Pin Icon', 'albedo'),
									'desc' => __('Add Marker Pin image', 'albedo'),
									'type' => 'upload',
								),
								'thumb' => array(
									'label'       => __('Location Image', 'albedo'),
									'desc'        => __('Add location image', 'albedo'),
									'type'        => 'upload',
								)
							)
						)
	...
	public function _callback_get_custom_locations($atts) {
		...
		if (!empty($rows)) {
			foreach($rows as $key => $row) {
				...
				$result[$key]['icon']  			 = fw_resize(wp_get_attachment_url(fw_akg('icon/attachment_id', $row)), 64, 64, true);
	...
	protected function _render($atts, $content = null, $tag = '')
	{
		if (!isset($atts['data_provider']['population_method'])) {
			trigger_error(
				__('No location provider specified for map shortcode', 'albedo')
			);
			return '<b>' . __( 'Map Placeholder', 'albedo' ) . '</b>';
		}
		...
		if (!isset($this->data[$provider])) {
			return '<!-- WARNING: '
						 . sprintf(__('Unknown location provider "%s" specified for map shortcode', 'albedo'), $provider)
						 . ' -->';
		}
