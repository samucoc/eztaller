<?php
	...
	// Init the widget
	add_action( 'widgets_init', 'wplab_albedo_social_text_widget_register' );

	function wplab_albedo_social_text_widget_register() {
		register_widget( 'wplab_albedo_social_text_widget' );
	}

	class wplab_albedo_social_text_widget extends WP_Widget {

		/**
		 * Widget constructor
		 **/
		function __construct() {
			$widget_ops = array( 'classname' => 'wproto_social_text_widget', 'description' => esc_html__('A widget that displays social icons and free text', 'albedo') );

			$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'wproto_social_text_widget' );

			parent::__construct( 'wproto_social_text_widget', esc_html__( '[ALBEDO FOOTER] Minimal: Social Icons and Text', 'albedo' ), $widget_ops, $control_ops );
		...
		**/
		function add_styles() {
			if( is_active_widget( false, false, $this->id_base, true ) ) {
				...
				wp_enqueue_style( 'wplab-albedo-widget-social-text-variable', wplab_albedo_utils::locate_uri( '/core/widget/social-text/style_variable.less'), false, _WPLAB_ALBEDO_CACHE_TIME_ );
			}
		}
