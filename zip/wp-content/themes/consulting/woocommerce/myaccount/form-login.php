	...
	<div class="u-column2 col-2">
		...
		<form method="post" class="register">
			...
			<?php if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) : ?>
				...
				<p class="form-row form-row-wide">
					<input type="text" class="input-text form-control" placeholder="<?php _e( 'Username', 'consulting' ); ?>" name="username" id="reg_username" value="<?php echo ( ! empty( $_POST['username'] ) ) ? esc_attr( $_POST['username'] ) : ''; ?>" />
				</p>

			<?php endif; ?>

			<p class="form-row form-row-wide">
				<input type="email" class="input-text form-control" placeholder="<?php _e( 'Email address', 'consulting' ); ?>" name="email" id="reg_email" value="<?php echo ( ! empty( $_POST['email'] ) ) ? esc_attr( $_POST['email'] ) : ''; ?>" />
			</p>

			<?php if ( 'no' === get_option( 'woocommerce_registration_generate_password' ) ) : ?>

				<p class="form-row form-row-wide">
					<input type="password" class="input-text form-control" placeholder="<?php _e( 'Password', 'consulting' ); ?>" name="password" id="reg_password" />
				</p>
