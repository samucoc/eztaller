...
$fecha_buscar 	= $_GET['fecha_buscar'];
				...
				$sql_boletas = "select concat(PaternoAlumno,' ',MaternoAlumno,' ', NombresAlumno) as alumno, 
								...
								sum(ValorPactado)-sum(ValorPagado) as saldo, 
						...
						where alumnos".$anio.".NumeroRutAlumno = '".$ncorr."' 

								and FechaVencimiento <= '".$fecha_buscar."'

						 		and CuentaCorriente".$anio.".CodigoItem > 1 
				...
				while($row_boletas = mysql_fetch_array($res_boletas)){
					...
					if ($row_boletas['saldo']!='0'){

						$pdf->Row(array(utf8_decode('Colegiatura'),utf8_decode($row_boletas['FechaVencimiento_1']),utf8_decode($row_boletas['ValorPactado']),utf8_decode($row_boletas['ValorPagado']),utf8_decode($row_boletas['saldo'])),0);
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$rut."_aviso_cobranza.pdf","F");

	$path = 'pdfs/'.$fecha.'_'.$rut."_aviso_cobranza.pdf";  	
        ...
        $correo->From = 'administracion@nmva.cl';
		...
		$email_apoderado = $_GET['email'];
 		...
 		//$email_apoderado = 'samu.silva@gmail.com';
        ...
        $correo->Subject = utf8_decode('Sistema Gestión Escolar - '.$nombre_establecimiento.' - Aviso de Cobranza');         
        ...
        $body .= "<p>Señor Apoderado:</p>";

		$body .= "<p>Adjunto envío aviso de cobranza año ".$anio.".</p>";

		$body .= "<p>Atentamente, </p>";

		$body .= "<p>Departamento de Cobranza</p>";

		$body .= "<p>$nombre_establecimiento</p>";
