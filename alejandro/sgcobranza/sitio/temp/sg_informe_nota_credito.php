...
$xajax->setRequestURI("sg_informe_nota_credito.php");
...
function Grabar($data){
	...
	$objResponse->addScript("document.getElementById('divlistado').style.display='none';");
        ...
        if ($nc != ''){
            $and .= " and nro_nota_credito = '".$nc."' " ;
        }
		  if ($tipo != ''){
			$and .= " and tipo_nc = '".$tipo."' " ;
			}
      if ($mes != ''){
			$and .= " and mes = '".$mes."' " ;
        	}
	...
	if (mysql_num_rows($res_ab) > 0){
		...
		while ($line_ab_1 = mysql_fetch_array($res_ab)){
			...
			array_push($arrRegistros, array("item"			=>	$i,
											...
											"nro_nc"   		=> 	$line_ab_1['nro_nota_credito'],
											"nro_boleta"   	=> 	$line_ab_1['nro_factura'],
											"observacion"  	=> 	$line_ab_1['observacion'],
											"neto"   		=> 	$line_ab_1['neto'],
											"iva"   		=> 	$line_ab_1['iva'],
											"total"   		=> 	$line_ab_1['total'],
											"tipo"   		=> 	$line_ab_1['tipo_nc'],
											"cierre"		=>  $ingresa 
											));
			$i++;
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_nota_credito_list.tpl'));
...
function Eliminar($data,$ncorr){
	...
	$sql ="delete from nc_periodo where nro_nota_credito = ".$ncorr;
	$res = mysql_query($sql,$conexion);
...
$miSmarty->display('sg_informe_nota_credito.tpl');
