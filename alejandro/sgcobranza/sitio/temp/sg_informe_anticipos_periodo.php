...
$xajax->setRequestURI("sg_informe_anticipos_periodo.php");
...
function Grabar($data){
	...
	if (($trabajador != '') && ($trabajador != 'Todos')){
	...
	if (($OBLItxtFecha1 != '') && ($OBLItxtFecha2 != '') ){
		list($dia3,$mes3,$anio3) = split('[/.-]', $OBLItxtFecha1);$OBLItxtFecha1 = $anio3."-".$mes3."-".$dia3;
		list($dia3,$mes3,$anio3) = split('[/.-]', $OBLItxtFecha2);$OBLItxtFecha2 = $anio3."-".$mes3."-".$dia3;
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_array($res_pd)) {
			...
			array_push($arrRegistros, array("centro_costo" 		=> 	$line_pd['centro_costo'],
											"fecha"  			=> 	$line_pd['fecha'],
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_anticipos_periodo_list.tpl'));
...
$miSmarty->display('sg_informe_anticipos_periodo.tpl');
