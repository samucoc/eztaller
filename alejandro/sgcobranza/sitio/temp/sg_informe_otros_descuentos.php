...
$xajax->setRequestURI("sg_informe_otros_descuentos.php");
...
function Grabar($data){
	...
	if (($OBLItxtFecha1 != '') && ($OBLItxtFecha2 != '') ){
		...
		$and .= " and otros_descuentos.fecha between '".$OBLItxtFecha1."' and '".$OBLItxtFecha2."' ";
	}
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_array($res_pd)) {
			...
			array_push($arrRegistros, array("tipo"  			=> 	$line_pd['tipo'],
											"fecha"  			=> 	$line_pd['fecha'],
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_otros_descuentos_list.tpl'));
...
$miSmarty->display('sg_informe_otros_descuentos.tpl');
