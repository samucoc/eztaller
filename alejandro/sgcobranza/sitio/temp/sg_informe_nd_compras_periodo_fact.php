...
$xajax->setRequestURI("sg_informe_nd_compras_periodo_fact.php");
...
function Asociar($data){
	...
	if ($ingresa == 'SI'){
		for ($i=0; $i<count($facturas);$i++){
			$sql = "select tipo_factura from facturas where nro_factura = '".$facturas[$i]."'";
			$res = mysql_query($sql,$conexion) or die(mysql_error());
			$row = mysql_fetch_array($res);
			$tipo_factura =$row['tipo_factura'];
			$sql = "insert into facturas_periodo(`nro_factura`, `mes`, `anio`, tipo) values('".$facturas[$i]."','".$cier_mes."','".$cier_anio."','".$tipo_factura."')";
			$res = mysql_query($sql,$conexion) or die(mysql_error());
...
function Grabar($data){
	...
	$sql_ab = "select *
                    ...
                    where tipo_nd = 'COMPRAS' and
						nro_factura not in (select nro_factura
											from nd_periodo)
					        ".$and_1." ".$and_2." ".$and_3."";
	...
	if (mysql_num_rows($res_ab) > 0){
		...
		while ($line_ab_1 = mysql_fetch_array($res_ab)){
			...
			$sql_pr = "select raz_social from proveedores where proveedor_ncorr  = '".$line_ab_1['rut']."'";
			$res_pr = mysql_query($sql_pr, $conexion);
			...
			array_push($arrRegistros, array("item"			=>	$i,
											...
											"tipo_factura"	=> 	$line_ab_1['tipo_factura'],
											"fecha"   		=> 	$line_ab_1['fecha'],
											"cliente" 		=> 	$nombre_persona,
											"nro_nd"   		=> 	$line_ab_1['nro_nota_debito'],
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_nd_compras_periodo_fact_list.tpl'));
...
$miSmarty->display('sg_informe_nd_compras_periodo_fact.tpl');
