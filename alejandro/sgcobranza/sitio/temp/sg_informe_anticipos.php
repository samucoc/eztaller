...
$xajax->setRequestURI("sg_informe_anticipos.php");
...
function Grabar($data){
	...
	if (($centro_costo != '')&&($centro_costo != '- - Seleccione - -')){
		$and .= " and anticipos.centro_costo = '".$centro_costo."' ";
	}
	if (($tt != '')&&($tt != 'Todos')){
		$and .= " and anticipos.tipo_trab = '".$tt."' ";
	}
	
	if (($grupo != '')){
		$and .= " and anticipos.grupo = '".$grupo."' ";
	}
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_array($res_pd)) {
			$sql_1 = "select concat(apellido_pat,' ',apellido_mat,' ',nombres) as nombres,dv
			...
			$sql_1 = "select nombre	
						from tipos_trabajadores
						where tt_ncorr = ".$line_pd['tipo_trab'];
			$res_1 = mysql_query($sql_1,$conexion) or die(mysql_error());
			$row_1 = mysql_fetch_array($res_1);
			
			$tipo_trabajador = $row_1['nombre'];
			
			array_push($arrRegistros, array("fecha"  => 	$line_pd['fecha'],
							"rut_trab"	=> 	$line_pd['trabajador'].'-'.$dv,
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_anticipos_list.tpl'));
...
$miSmarty->display('sg_informe_anticipos.tpl');
