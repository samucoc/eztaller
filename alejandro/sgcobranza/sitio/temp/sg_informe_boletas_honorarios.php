...
$xajax->setRequestURI("sg_informe_boletas_honorarios.php");
...
function Grabar($data){
	...
	if (mysql_num_rows($res_ab) > 0){
		...
		while ($line_ab_1 = mysql_fetch_array($res_ab)){
			...
			$sql_pr = "select razon_social from prestadores where prestador_ncorr  = '".$line_ab_1['prestador']."'";
			$res_pr = mysql_query($sql_pr, $conexion);
			$nombre_persona = @mysql_result($res_pr,0,"razon_social");
			...
			array_push($arrRegistros, array("item"			=>	$i,
											"ncorr" 		=> 	$line_ab_1['bh_ncorr'],
											"empresa"  		=> 	$nombre_empresa,
											"fecha"   		=> 	$line_ab_1['fecha'],
											"cliente" 		=> 	$nombre_persona,
											"nro_boleta"   	=> 	$line_ab_1['nro_boleta'],
											"neto"   		=> 	$line_ab_1['neto'],
											"iva"   		=> 	$line_ab_1['retencion'],
											"total"   		=> 	$line_ab_1['total'],
											"cierre"		=>  $ingresa 
											));
			$i++;
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_boletas_honorarios_list.tpl'));
...
function Eliminar($data,$ncorr){
	...
	$sql ="delete from boletas_honorarios where bh_ncorr = ".$ncorr;
	$res = mysql_query($sql,$conexion);
...
$miSmarty->display('sg_informe_boletas_honorarios.tpl');
