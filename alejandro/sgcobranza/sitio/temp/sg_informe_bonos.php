...
$xajax->setRequestURI("sg_informe_bonos.php");
...
function Grabar($data){
	...
	if (($OBLItxtFecha1 != '')){
		...
		$and .= " and bonos.fecha between '".$OBLItxtFecha1."' and '".$OBLItxtFecha2."' ";
	}

	if (($centro_costo != '')&&($centro_costo != '- - Seleccione - -')){
		$and .= " and bonos.centro_costo = '".$centro_costo."' ";
	}
	
	if (($grupo != '')){
		$and .= " and bonos.grupo = '".$grupo."' ";
	}
	...
	if (mysql_num_rows($res_pd) > 0){
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_informe_bonos_list.tpl'));
...
function CargaPagina($data){
	...
	$objResponse->addScript("xajax_CargaSelect(xajax.getFormValues('Form1'),'cboTipo','tipo_bonos','','Todos','ta_ncorr','nombre', '')");
...
$miSmarty->display('sg_informe_bonos.tpl');
