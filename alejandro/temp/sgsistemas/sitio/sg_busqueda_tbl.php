...
function Busca($data, $opt){
	...
	if ($tbl == 'cuentas'){
		...
		if ($opt == '2'){$campo = "CaNumero";	$texto = $desc;}
		
		$sql = "select CaId, CaNumero from $tbl where $campo like '".$texto."%' order by CaId asc";
	}
...
function CargaPagina($data){
	...
	if ($tbl == 'cuentas'){
		$sql = "select CaId, CaNumero from $tbl order by CaId asc";
	}
