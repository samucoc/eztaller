...
$xajax->setRequestURI("sg_existencia_precontrol.php");
...
function Grabar($data){
	...
	$sql_cobr			=	"select ve_vendedor from sgbodega.vendedores where ve_codigo = '".$cobrador."'";
	$res_cobr			= 	mysql_query($sql_cobr,$conexion);
	$row_cobr			=	mysql_fetch_array($res_cobr);
	$nombre_cobrador 	=	$row_cobr["ve_vendedor"];
	...
	$nro_guia_inicial = @mysql_result($res,0,"numero");
	$ult_fecha = @mysql_result($res,0,"gd_fecha");
	...
	if($fecha_hasta!=''){
		...
		if (mysql_num_rows($res_pd) > 0){
				...
				if ($codigo != ''){
					...
					*/
					...
					// busca el stock inicial de la ultima guia y el codigo
						$sql_sguia = "SELECT b.gd_fecha AS gd_fecha, a.sg_stocknuevo AS inicial_n, a.sg_stockusado AS inicial_u
										FROM sgbodega.subgdcontrol a
										INNER JOIN sgbodega.gdcontrol b ON a.sg_numero = b.gd_numero
										WHERE a.sg_codbus =  '".$codigo."'
										AND a.sg_vendedor =  '".$cobrador."'
										AND a.sg_numero =  '".$nro_guia_inicial."'
										ORDER BY a.sg_numero DESC 
										LIMIT 0 , 1";
						...
						//echo $ult_fecha = @mysql_result($res_sguia, 0, "gd_fecha");
					
					$TOTAL_INICIAL_N = $inicial_n;
					$TOTAL_INICIAL_U = $inicial_u;
					//fin
					...
					//busca los aumentos en el periodo (movim 2)
						...
						$sql_saumentos 	= "select a.mdet_nu as nu, a.mdet_cantidad as cantidad
											...
											b.movim_ncorr  in (select guia from sgyonley.control_guias) and
					...
					//busca los aumentos que hace existencia (movim 6)
						...
						$TOTAL_AUMENTOS_NB = $TOTAL_AUMENTOS_NB + $aumentos_nb;
						$TOTAL_AUMENTOS_UB = $TOTAL_AUMENTOS_UB + $aumentos_ub;
					//fin
				...
				if ($codigo != ''){
					if ($filtro == '1'){
						array_push($arrRegistros, array("item"				=>	$i,
														...
														"aumentos_nb" 		=> 	$aumentos_nb,
														"aumentos_ub" 		=> 	$aumentos_ub,
														"ventas_n" 			=> 	$ventas_n,
														"ventas_u" 			=> 	$ventas_u,
														"rebajas_n" 		=> 	$dv_nuevo,
														"rebajas_u" 		=> 	$dv_usado,
			...
			*/
			...
			$_SESSION["alycar_total_aumentos_nb"] 	= 	$TOTAL_AUMENTOS_NB;
			$_SESSION["alycar_total_aumentos_ub"] 	= 	$TOTAL_AUMENTOS_UB;
			...
			$miSmarty->assign('TOTAL_AUMENTOS_NB', $TOTAL_AUMENTOS_NB);
			$miSmarty->assign('TOTAL_AUMENTOS_UB', $TOTAL_AUMENTOS_UB);
		...
		$objResponse->addAlert("Ingrese periodo con cierres");
		}	
...
function Ordenar($data, $campo){
	...
	$miSmarty->assign('TOTAL_AUMENTOS_NB', $_SESSION["alycar_total_aumentos_nb"]);
	$miSmarty->assign('TOTAL_AUMENTOS_UB', $_SESSION["alycar_total_aumentos_ub"]);
	...
	$miSmarty->assign('arrRegistros', $arrRegistros);
	$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_existencia_precontrol_list.tpl'));
...
function OrdenarRevision($data, $campo){
	...
	*/
	...
	$miSmarty->assign('arrRegistrosRev', $arrRegistrosRev);
	$objResponse->addAssign("divrevision", "innerHTML", $miSmarty->fetch('sg_existencia_precontrol_revision_list.tpl'));
...
function LlamaDetalle($data, $codigo, $descripcion){
	...
	$objResponse->addScript("showPopWin('sg_existencia_movimientos_vendedor_detalle.2.php?codigo=$codigo&descripcion=$descripcion&fecha1=$fecha1&fecha2=$fecha2&cobrador=$cobrador&empresa=$empresa&mes=$mes&anio=$anio', '$codigo $descripcion', 700, 280, null);");
...
$miSmarty->assign('COD_VENDEDOR', $_GET["vendedor"]);

$miSmarty->display('sg_existencia_precontrol.tpl');
