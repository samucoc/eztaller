...
function Grabar($data){
	...
	$cier_bodega		=	$_SESSION["alycar_sgyonley_bodega"];
	...
	if ($ingresa == 'SI'){
		$sql = "insert into cierres
				(cier_bodega,cier_fecha,cier_usuario,cier_fechadig, cier_mes, cier_anio)
...
function MuestraUltCierre($data){
	...
	$sql = "select DATE_FORMAT(cier_fecha,'%d/%m/%Y') as ultimocierre from cierres where trab_ncorr = '".$trab_ncorr."' order by cier_fecha desc limit 1";
	$res = mysql_query($sql, $conexion);
