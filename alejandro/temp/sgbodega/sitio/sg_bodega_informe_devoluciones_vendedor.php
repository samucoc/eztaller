...
$xajax->setRequestURI("sg_bodega_informe_devoluciones_vendedor.php");
...
function Grabar($data){
	...
	$vend = 'Todos';
	if (($cod_vendedor != '') && ($nombre_vendedor != '')){
		$and = "a.vend_ncorr = '".$cod_vendedor."' and ";
		$vend	=	$cod_vendedor." ".$nombre_vendedor;
	}
	...
	$sql = "select
				...
				from 
				sgcompras.movim a, sgcompras.movim_detalle c, sgyonley.empresas d
					
				where
				a.movim_bodega = '".$movim_bodega."' and
	...
	if (mysql_num_rows($res) > 0){
		...
		while ($line = mysql_fetch_row($res)) {
			//busca al vendedor
			$sql_pr = "select ve_vendedor from sgbodega.vendedores where ve_codigo = '".$line[3]."'";
			$res_pr = mysql_query($sql_pr, $conexion);
			$vendedor = @mysql_result($res_pr,0,"ve_vendedor");
		...
		$miSmarty->assign('VENDEDOR', $vend);
		$miSmarty->assign('MOVIMIENTO', $movimiento);
		$miSmarty->assign('TOTAL_MOVIM', $i - 1);
		$miSmarty->assign('arrRegistros', $arrRegistros);
		
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_bodega_informe_devoluciones_vendedor_list.tpl'));
	}else{
...
function CargaPagina($data){
	...
	if (($_SESSION["alycar_fecha_desde"] != '') && ($_SESSION["alycar_fecha_hasta"] != '')){
		...
		$objResponse->addAssign("txtCodCobrador", "value", $_SESSION["alycar_cod_vendedor"]);
		$objResponse->addAssign("txtDescCobrador", "value", $_SESSION["alycar_nombre_vendedor"]);
...
function LlamaGuia($data, $guia){
	...
	$_SESSION["alycar_pag_regreso"] 		= 	"sg_bodega_informe_devoluciones_vendedor.php";
...
$miSmarty->display('sg_bodega_informe_devoluciones_vendedor.tpl');
