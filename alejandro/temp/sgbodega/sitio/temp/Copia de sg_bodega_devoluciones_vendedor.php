...
function ConfirmaIngreso($data){
	...
	if ($ingresa == 'SI'){
		...
		if (mysql_num_rows($res_det) > 0){
			while ($line_det = mysql_fetch_row($res_det)) {
				...
				if (mysql_num_rows($res_p) > 0){
					$BV_STOCKNUEVO = @mysql_result($res_p,0,"BV_STOCKNUEVO") - $BV_STOCKNUEVO;
					$BV_STOCKUSADO = @mysql_result($res_p,0,"BV_STOCKUSADO") - $BV_STOCKUSADO;
		...
		$objResponse->addScript("alert('Devolución de Vendedor Ingresada Correctamente, El Nº Asignado es: $movim_ncorr')");
