...
function CargaListado($data){
	...
	if ($v_inicio != '' && $v_termino != ''){
		list($dia1,$mes1,$anio1) = split('[/.-]', $v_inicio);$fecha1 	= $anio1."-".$mes1."-".$dia1;
		list($dia2,$mes2,$anio2) = split('[/.-]', $v_termino);$fecha2 	= $anio2."-".$mes2."-".$dia2;
			...
			while ($line = mysql_fetch_row($res)) {
					...
					while ($line2 = mysql_fetch_row($res2)) {
						if ($encontrado == 'NO'){
							...
							if (@mysql_num_rows($res_cl) > 0){
								sgyonley.$tabla_clientes =	$tabla_cli;
								$encontrado = 'SI';
							}
						}
					}
				...
				if (($filtro_estado == $estado_venta) OR ($filtro_estado == $estado_venta_doble) OR ($filtro_estado == 'TODAS')){
					...
					$sql_busqueda_1 = "SELECT * 
											FROM  sgyonley.sgyonley.cargas_eliminadas 
