...
function CargaListado($data){
	...
	if ($cod_vendedor != '' && $vendedor != ''){
		...
		$sql = "select
				...
				EXISTS
				(SELECT caut_folio FROM sgyonley.cargas_autorizadas 
				WHERE 
				caut_empresa = '".$empresa."' and 
				caut_folio = sgyonley.ventas_antigua.vent_num_folio )
		...
		if (mysql_num_rows($res) > 0){
			...
			while ($line = mysql_fetch_row($res)) {
				...
				//detecta si el folio tiene todo entregado
					...
					$total_autorizado = 0;
					$sql_taut = "select vdet_ncorr from sgyonley.cargas_autorizadas  where caut_empresa = '".$empresa."' and caut_folio = '".$folio."'";
					...
					while ($line_taut = mysql_fetch_row($res_taut)) {
							...
							$sql_tv = "select vent_cant from sgyonley.ventas_detalle_antigua where vdet_ncorr = '".$vdet_ncorr."'";
