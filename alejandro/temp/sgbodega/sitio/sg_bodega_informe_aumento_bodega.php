...
$xajax->setRequestURI("sg_bodega_informe_aumento_bodega.php");
...
function Grabar($data){
	...
	if (mysql_num_rows($res) > 0){
		...
		while ($line = mysql_fetch_row($res)) {
			//busca al proveedor
			$sql_pr = "select pr_razon from sgcompras.proveedor where pr_rut = '".$line[3]."'";
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_bodega_informe_aumento_bodega_list.tpl'));
	}else{
...
function LlamaGuia($data, $guia){
	...
	$_SESSION["alycar_pag_regreso"] 		= 	"sg_bodega_informe_aumento_bodega.php";
...
$miSmarty->display('sg_bodega_informe_aumento_bodega.tpl');
