...
*/	############	FIN		################################################################################################
	...
	if ($ingresa == 'SI'){
		...
		if (!isset($_SESSION["alycar_sgyonley_aumento_vendedor"])){
			$sql = "insert into sgcompras.movim (movim_bodega,movim_tipo,empe_rut,movim_fecha,vend_ncorr,movim_obs,usu_id)
					values (
					'".$movim_bodega."','".$movim_tipo."','".$empe_rut."','".$movim_fecha."','".$vend_ncorr."','".$movim_obs."','".$usu_id."')";
...
function ConfirmaIngreso($data){
	...
	if ($ingresa == 'SI'){
		...
		$_SESSION["alycar_pag_regreso"] = 'sg_bodega_aumentos_vendedor.php?ncorr='.$_SESSION["alycar_sgyonley_aumento_vendedor"];
		unset($_SESSION["alycar_sgyonley_aumento_vendedor"]);
...
function CargaDesc($data, $tabla, $campo1, $campo2, $objeto1, $objeto2, $c_and){
	...
	if (($tabla == 'sgbodega.tallas') OR ($tabla == 'sgbodega.tallasnew')){
		...
		$sql = "select $campo2 as descripcion 
				from $tabla 
				where $campo1 = '".$ncorr."'  and VE_EMPRESA = '".$empresa."'";
