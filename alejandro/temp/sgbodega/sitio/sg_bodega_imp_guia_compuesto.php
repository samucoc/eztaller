...
$xajax->setRequestURI("sg_bodega_imp_guia_compuesto.php");
...
function Grabar($data){
	...
	if (mysql_num_rows($res) > 0){
		...
		$sql = "select mdet_ncorr, mdet_codigo, mdet_desc, mdet_nu, mdet_valor, sum(mdet_cantidad), mdet_subneto, mdet_descuento, mdet_subtotal
				from sgcompras.movim_detalle
				where movim_ncorr in (".substr($guia_actual,1).")
				$and
				group by mdet_codigo
				order by mdet_desc";
		...
		if (($movim_tipo == 2) OR ($movim_tipo == 4) OR ($movim_tipo == 6)){
			$sql_bdet = "select ve_vendedor from sgbodega.vendedores where ve_codigo = '".$vendedor."' ";
			$res_bdet = mysql_query($sql_bdet, $conexion) or die(mysql_error());
			$vendedor = $vendedor.' - '.@mysql_result($res_bdet, 0, "ve_vendedor" );
		}
		
		//busca al trabajador
		$trabajador="";
		if (($movim_tipo == '9')||($movim_tipo == '10')){
			$sql_trab = "select tra_nombre 
...
$miSmarty->display('sg_bodega_imp_guia_compuesto.tpl');
