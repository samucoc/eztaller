	...
	if (mysql_num_rows($res_pd) > 0){
		...
		while ($line_pd = mysql_fetch_row($res_pd)) {
			...
			$sql_trasp_tras	= "select  mdet_cantidad as cantidad
						...
						b.movim_tipo = '8' and
						b.movim_estado = 'FINALIZADO' and
						b.movim_bodega_tras = '".$movim_bodega."'  
						";
			...
			$stock_nuevo = $aumentos_n + $trasp_n - $trasp_n_tras + $dev_vend_n + $dev_clie_n_new - $aum_vend_n - $dev_pro_n - $desc_cp_n + $ajuste_aumentos_n  - $desc_merma_n  - $desc_castigo_n ;
			...
			//$objResponse->addAlert("$stock_nuevo");
...
function CargaDesc($data, $tabla, $campo1, $campo2, $objeto1, $objeto2, $c_and){
	...
	$sql = "select concat(ta_busqueda,' ',ta_descripcion) as descripcion from $tabla where $campo1 = '".$ncorr."' $c_and";
