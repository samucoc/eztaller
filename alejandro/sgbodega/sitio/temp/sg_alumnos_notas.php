...
function CargaListado($data){
	...
	$i= 0;
	if ($semestre==''){
		$select_max  ="select Semestre
								from Periodos
						where AnoAcademico = '".$_SESSION["sige_anio_escolar_vigente"]."'
							";
		$res_max = mysql_query($select_max,$conexion)or die(mysql_error());
		while($row_max = mysql_fetch_array($res_max)){
				$semestre				=	$row_max['Semestre'];

		            $objResponse->addCreate("semestre","option",""); 		
					$objResponse->addAssign("semestre","options[".$i."].value", $semestre);
					$objResponse->addAssign("semestre","options[".$i."].text", $semestre.' Semestre');

				$i++;
			}
		$objResponse->addScript('document.getElementById("semestre").value='.$semestre);
		}
	$rut					=	$data['rut'];
	...
	if (mysql_num_rows($res_ve) > 0){
		...
		while ($line_ve = mysql_fetch_row($res_ve)) {
			...
			array_push($arrRegistrosDetalle, array("item"					=>	$j, 
															...
															"nota"					=>	number_format(round($promedio,1),1,".","," ), 
															"codigo_asignatura"			=>	$line_ve[0]));
