...
$xajax->setRequestURI("sg_matricula_actualizar_antiguo.php");
...
function Grabar($data){
	...
	if ($autoriza==1){
		// $select_notas = "update alumnos 
		// 				set CodigoCurso = '".$curso."'
		...
		$select_notas = "INSERT INTO alumnos".$anio_sup." (`NumeroRutAlumno`, `DigitoRutAlumno`, `PaternoAlumno`, `MaternoAlumno`, 
													...
													NuevoAntiguo, FotoAlumno, EMailAlumno)
						SELECT 	`NumeroRutAlumno`, `DigitoRutAlumno`, `PaternoAlumno`, `MaternoAlumno`, 
								`NombresAlumno`, `DireccionParticularAlumno`, `CiudadParticularAlumno`, 
								`TelefonoParticularAlumno`, '".$curso."', '".date("Y-m-d")."', `FechaNacimiento`, 
								`SexoAlumno`, `OtroColegio`, `ColegioAnterior`, `CodigoComuna`, NumeroRutApoderado,'1',
								FotoAlumno, EMailAlumno
						FROM alumnos".$anio." WHERE NumeroRutAlumno = '".$rut_alumno."'";
		$res_notas = mysql_query($select_notas, $conexion);
		
		$select_notas = "update alumnos".$anio." set MatriculadoProxPeriodo = '1' WHERE NumeroRutAlumno = '".$rut_alumno."';";
		$res_notas = mysql_query($select_notas, $conexion);
		...
		$select_notas = "delete from alumnos".$anio_sup." WHERE NumeroRutAlumno = '".$rut_alumno."';";
		$res_notas = mysql_query($select_notas, $conexion);
		
		$select_notas = "update alumnos".$anio." set MatriculadoProxPeriodo = '0' WHERE NumeroRutAlumno = '".$rut_alumno."';";
		$res_notas = mysql_query($select_notas, $conexion);
...
function CargaPagina($data){
	...
	$sql = "select  distinct concat(`PaternoAlumno`,' ',`MaternoAlumno`,' , ',`NombresAlumno`) as nombre_alumno,
					NombreCurso, Cursos.CodigoCurso, Matriculado
				from alumnos".$anio."
					inner join Cursos
	...
	$objResponse->addAssign('autoriza','value',$row['Matriculado']);
		
	$codigo_curso = $row['CodigoCurso'];

	// // carga empresas
	// $where = '"%ADMISION%"';
	// $objResponse->addScript("xajax_CargaSelect(xajax.getFormValues('Form1'),'cboCurso','Cursos','','- - Seleccione - -','CodigoCurso','NombreCurso', ' where NombreCurso not like ".$where."')");
	
	$sql_1  ="select a.NombreCurso, a.CodigoCurso 
				from Cursos b , Cursos a
				where b.CodigoCurso = '".$codigo_curso."' and 
					a.CodigoCurso = b.CursoPrecede
				";
	$res_1 = mysql_query($sql_1,$conexion);
	$row_1 = mysql_fetch_array($res_1);

	$objResponse->addAssign('cboCurso','value',$row_1['CodigoCurso']);
	$objResponse->addAssign('nombre_curso','value',$row_1['NombreCurso']);
...
function CargaListado($data){
	...
	if (mysql_num_rows($res_ve) > 0){
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_matricula_actualizar_antiguo_list.tpl'));
...
$miSmarty->display('sg_matricula_actualizar_antiguo.tpl');
