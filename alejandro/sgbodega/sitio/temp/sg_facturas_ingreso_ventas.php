...
$xajax->setRequestURI("sg_facturas_ingreso_ventas.php");
...
function Grabar($data){
	...
	if (mysql_num_rows($res)==0){
		...
		if ($rut != ''){
			$sql = "insert into `facturas` (`fecha`, empresa,`rut`, `nro_factura`,`clase_factura`, `neto`, `iva`, `total`, `tipo_factura`, `estado_factura`) values 
				('".$fecha."','".$empresa."','".$rut."','".$nro_factura."','".$clase_factura."','".$neto."','".$iva."','".$total."','VENTAS','".$estado_factura."')";
			$res = mysql_query($sql, $conexion);
			$objResponse->addScript("alert('Registro guardado correctamente')");
			$objResponse->addScript("document.Form1.submit();");
			}
		else{
			$objResponse->addScript("alert('Agregar Cliente')");
			}
		}
	}
	else{
...
function LlamaMantenedorVxC($data){
	...
	$_SESSION["alycar_pagina_volver"]	=   'sg_facturas_ingreso_ventas.php';
...
$miSmarty->display('sg_facturas_ingreso_ventas.tpl');
