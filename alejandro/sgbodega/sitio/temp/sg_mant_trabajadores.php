			...
			while ($i < $j) {
				...
				if (!isset($data[$objeto])){
					...
					if (!isset($data[$objeto])){
						...
						if ("CHK".$arrCampos[$i]=='CHKvehiculos'){
							$objResponse->addScript("xajax_CambiarVrhiculos(xajax.getFormValues('Form1'))");
							}
						elseif ("CHK".$arrCampos[$i]=='CHKfondos_rendir'){
							$objResponse->addScript("xajax_CambiarFxR(xajax.getFormValues('Form1'))");
							}
						elseif ("CHK".$arrCampos[$i]=='CHKcuentas_personales'){
							$objResponse->addScript("xajax_CambiarCuentasPersonales(xajax.getFormValues('Form1'))");
							}
						}
					}
...
function CambiarCuentasPersonales($data){
    global $conexion;
	$objResponse = new xajaxResponse('ISO-8859-1');

	$check = $data['CHKcuentas_personales'];
	$ncorr = $data['txtNcorr'];
	
	//borrar cp
	if ($ncorr!=''){
		if (($check == 'on')){
			$sql_1 = "select * from yonleycp.trabajadores
					where tra_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
			$res_1 = mysql_query($sql_1,$conexion) or die(mysql_error());
			if (mysql_num_rows($res_1)>0){
				$sql = "update yonleycp.trabajadores
							set tra_nombre = '".$data['OBLInombres']." ".$data['OBLIapellido_pat']." ".$data['OBLIapellido_mat']."',
								tra_rut = '".$data['OBLIrut']."'
						where tra_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
				$res = mysql_query($sql_1,$conexion) or die(mysql_error());
				}
			else{
				$sql = "insert into yonleycp.trabajadores (tra_nombre,tra_rut)
						values ('".$data['OBLInombres']." ".$data['OBLIapellido_pat']." ".$data['OBLIapellido_mat']."','".$data['OBLIrut']."')
					";
				$res = mysql_query($sql,$conexion) or die(mysql_error());
				}
			}
		else{
			$sql = "delete from yonleycp.trabajadores
					where tra_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
			$res = mysql_query($sql,$conexion) or die(mysql_error());
			}
		}
...
function CambiarVehiculos($data){
    global $conexion;
	$objResponse = new xajaxResponse('ISO-8859-1');

	$check = $data['CHKvehiculos'];
	$ncorr = $data['txtNcorr'];
	
	//borrar cp
	if ($ncorr!=''){
		if (($check == 'on')){
			$sql_1 = "select * from sgcopec.personas
					where pers_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
			$res_1 = mysql_query($sql_1,$conexion) or die(mysql_error());
			if (mysql_num_rows($res_1)>0){
				$sql = "update sgcopec.personas
							set pers_nombre = '".$data['OBLInombres']."',
								pers_ape_pat = '".$data['OBLIapellido_pat']."',
								pers_ape_mat = '".$data['OBLIapellido_mat']."',
								pers_rut = '".$data['OBLIrut']."'
						where pers_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
				$res = mysql_query($sql_1,$conexion) or die(mysql_error());
				}
			else{
				$sql = "insert into sgcopec.personas (pers_nombre,pers_ape_pat,pers_ape_mat,pers_rut)
						values ('".$data['OBLInombres']."','".$data['OBLIapellido_pat']."','".$data['OBLIapellido_mat']."','".$data['OBLIrut']."')
					";
				$res = mysql_query($sql,$conexion) or die(mysql_error());
				}
			...
			$sql = "delete from sgcopec.personas
					where pers_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
			$res = mysql_query($sql,$conexion) or die(mysql_error());
			}
		}
...
function CambiarFxR($data){
    global $conexion;
	$objResponse = new xajaxResponse('ISO-8859-1');

	$check = $data['CHKfondos_rendir'];
	$ncorr = $data['txtNcorr'];
	
	//borrar cp
	if ($ncorr!=''){
		if (($check == 'on')){
			$sql_1 = "select * from sgcaja.trabajadores
					where trab_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
			$res_1 = mysql_query($sql_1,$conexion) or die(mysql_error());
			if (mysql_num_rows($res_1)>0){
				$sql = "update sgcaja.trabajadores
							set trab_nombres = '".$data['OBLInombres']."',
								trab_apellidos = '".$data['OBLIapellido_pat']." ".$data['OBLIapellido_mat']."',
								trab_rut = '".$data['OBLIrut']."'
						where trab_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
				$res = mysql_query($sql_1,$conexion) or die(mysql_error());
				}
			else{
				$sql = "insert into sgcaja.trabajadores (trab_nombres,trab_apellidos,trab_rut)
						values ('".$data['OBLInombres']."','".$data['OBLIapellido_pat']." ".$data['OBLIapellido_mat']."','".$data['OBLIrut']."')
					";
				$res = mysql_query($sql,$conexion) or die(mysql_error());
				}
			...
			$sql = "delete from sgcaja.trabajadores
					where trab_rut in (select rut from sggeneral.trabajadores where trabajador_ncorr	= '".$ncorr."')";
			$res = mysql_query($sql,$conexion) or die(mysql_error());
			}
		}
...
$xajax->registerFunction("CambiarCuentasPersonales");
$xajax->registerFunction("CambiarVehiculos");
$xajax->registerFunction("CambiarFxR");
