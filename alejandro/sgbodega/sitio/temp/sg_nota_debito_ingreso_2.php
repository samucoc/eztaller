...
$xajax->setRequestURI("sg_nota_debito_ingreso_2.php");
...
function Grabar($data){
	...
	$sql = "select * from notas_debito where nd_ncorr = ".$nro_nc;
	$res = mysql_query($sql,$conexion);
	if (mysql_num_rows($res)==0){
		$sql_1 = "select * from facturas where nro_factura = '".$nro_factura."' and tipo_factura = 'VENTAS'";
		$res_1 = mysql_query($sql_1,$conexion);
		if (mysql_num_rows($res_1)>0){
			if ($ingresa == 'SI'){
				$neto = str_replace(".", "", $neto);
				$sql = "insert into `notas_debito` ( `fecha`,  `empresa`, `rut`, `nro_nota_debito`, `nro_factura`, `neto`, `iva`, `total`, `observacion`, tipo_nd) values 
						('".$fecha."','".$empresa."','".$rut."','".$nro_nd."','".$nro_factura."','".$neto."','".$iva."','".$total."','".$observacion."','VENTAS')";
				$res = mysql_query($sql, $conexion) or die(mysql_error());
				$objResponse->addScript("alert('Registro guardado correctamente')");
				$objResponse->addScript("document.Form1.submit();");
				}
		else{
			$objResponse->addScript("alert('Factura no es de ventas')");
			}
		}
	}
	else{
		$objResponse->addScript("alert('Nota de Debito repetida')");
		}
...
function LlamaMantenedorVxC($data){
	...
	$_SESSION["alycar_pagina_volver"]	=   'sg_nota_debito_ingreso_2.php';
	
    $objResponse->addScript("document.location.href='sg_mant_tablas.php?tbl=clientes&nombre_empresa=".$nombre_empresa."&empresa=".$empresa."&fecha=".$fecha."&cliente=".$cliente."&rut=".$rut."&nro_factura=".$nro_factura."&neto=".$neto."&iva=".$iva."&total=".$total."'");
...
$miSmarty->display('sg_nota_debito_ingreso_2.tpl');
