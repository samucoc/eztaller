...
$pdf->SetAutoPageBreak(true,25);  
	...
	$pdf->Row(array(utf8_decode('MARCIA GONZÁLEZ HENRÍQUEZ, Directora del '.$nombre_colegio.' de Villa Alemana, RBD: '.$RBD.', certifica que don (a) '.$nombre_alumno.' R.U.T.: '.$rut.',  alumno (a) '.$nombre_curso.' periodo '.$anio.', asiste regularmente a clases en nuestro establecimiento.')),0);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	...
	$path = 'pdfs/'.$fecha.'_'.$alumno."_certificado_alumno_regular.pdf";  	
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Reporte Sistema Integrado de Gestion Escolar - Certificado de Alumno Regular');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = "Envio de Certificado de Alumno Regular";
