...
$pdf->Row(array(utf8_decode('____________________________________________________________________________________________________')),0);
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$rut."_notas_semestral.pdf","F");
	$path = 'pdfs/'.$fecha.'_'.$rut.."_notas_semestral.pdf";  	
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Reporte Sistema Integrado de Gestion Escolar - Notas Semestral Alumno');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = "Envio de Notas Semestral Alumno";
