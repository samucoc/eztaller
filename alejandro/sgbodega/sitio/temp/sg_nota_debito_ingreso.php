...
$xajax->setRequestURI("sg_nota_debito_ingreso.php");
		...
		if (mysql_num_rows($res_1)>0){
			if ($ingresa == 'SI'){
				...
				$sql = "insert into `notas_debito` ( `fecha`,  `empresa`, `rut`, `nro_nota_debito`, `nro_factura`, `neto`, `iva`, `total`, `observacion`, tipo_nd) values 
						('".$fecha."','".$empresa."','".$rut."','".$nro_nd."','".$nro_factura."','".$neto."','".$iva."','".$total."','".$observacion."','COMPRAS')";
				$res = mysql_query($sql, $conexion);
...
function LlamaMantenedorVxC($data){
	...
	$_SESSION["alycar_pagina_volver"]	=   'sg_nota_debito_ingreso.php';
...
$miSmarty->display('sg_nota_debito_ingreso.tpl');
