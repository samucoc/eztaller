...
$xajax->setRequestURI("sg_pruebas_gestion.php");
...
function CargaPagina($data){
	...
	//$objResponse->addScript("xajax_CargaSelect(xajax.getFormValues('Form1'),'profesores','Profesores','','- - Seleccione - -','NumeroRutProfesor','NumeroRutProfesor', '')");
...
function CargaListado($data){
	...
	if (($anio!='')&&($semestre!='')){
		...
		if (($CodigoCurso!='')&&($CodigoCurso!='Todos')){
			$and1 = " Pruebas.CodigoCurso = '".$CodigoCurso."' and ";
			}
		if (($Profesor!='')&&($Profesor!='Todos')){
			$and1 = " Pruebas.NumeroRutProfesor = '".$Profesor."' and ";
			}

		// busca los registros
		$sql_ve = "select  distinct concat(`PaternoProfesor`,' ',`MaternoProfesor`,' , ',`NombresProfesor`) as nombre_profesor,
							Ramos.Descripcion, ".$semestre.", FechaPrueba , 
							DATE_ADD(FechaPrueba, INTERVAL 10 DAY) as fecha_tope, DATEDIFF(FechaRealPrueba , FechaPrueba) as cant_dias, DescripcionPrueba,FechaRealPrueba
					from Pruebas
						inner join Profesores
							on Pruebas.NumeroRutProfesor = Profesores.NumeroRutProfesor
						inner join Asignaturas
							on Asignaturas.CodigoRamo = Pruebas.CodigoRamo
						inner join Ramos
		...
		if (mysql_num_rows($res_ve) > 0){
			...
			while ($line_ve = mysql_fetch_row($res_ve)) {
				array_push($arrRegistros, array("item"					=>	$i, 
												...
												"descripcion"			=> 	$line_ve[1],
												"semestre" 				=> 	$line_ve[2],
												"FechaPrueba" 			=> 	$line_ve[3],
												"fecha_tope" 			=> 	$line_ve[4],
												"cant_dias"				=> 	$line_ve[5],
												"nombre_prueba"			=> 	$line_ve[6],
												"FechaRealPrueba"		=> 	$line_ve[7] 
												));
				$i++;
				}
			
			$miSmarty->assign('arrRegistros', $arrRegistros);
			$miSmarty->assign('cant_filas', $i);
			$miSmarty->assign('anio', $anio);
			$miSmarty->assign('semestre', $semestre);

		    $sql = "select NombreCurso as descripcion from Cursos where CodigoCurso = '".$CodigoCurso."' ";
			$res = mysql_query($sql, $conexion);
			
			$miSmarty->assign('nombre_curso',  @mysql_result($res,0,"descripcion"));

		    $sql = "select concat(NombresProfesor,' ',PaternoProfesor,' ',MaternoProfesor) as descripcion from Profesores where NumeroRutProfesor = '".$Profesor."' ";
			$res = mysql_query($sql, $conexion);
			
			$miSmarty->assign('nombre_profesor',  @mysql_result($res,0,"descripcion"));

	
			$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_pruebas_gestion_list.tpl'));
...
$miSmarty->display('sg_pruebas_gestion.tpl');
