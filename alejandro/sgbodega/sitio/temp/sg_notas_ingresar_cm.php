...
$xajax->setRequestURI("sg_notas_ingresar_cm.php");
...
function CargaListado($data){
	...
	$sql_ve = "select  distinct concat(`PaternoAlumno`,' ',`MaternoAlumno`,' , ',`NombresAlumno`) as nombre_alumno,
						...
						Pruebas.prueba_ncorr, Matriculas.FechaRetiro
				from alumnos".$anio."
					inner join Cursos
	...
	if (mysql_num_rows($res_ve) > 0){
		...
		while ($line_ve = mysql_fetch_row($res_ve)) {
			array_push($arrRegistros, array("item"					=>	$i, 
											...
											"prueba"				=> 	$line_ve[5],
											"FechaRetiro"			=> 	$line_ve[6]
											));
			$i++;
			$select_notas = "select Nota,NumeroRutAlumno
							from NotasAlumnos
		...
		$objResponse->addAssign("divabonos", "innerHTML", $miSmarty->fetch('sg_notas_ingresar_cm_list.tpl'));
...
$miSmarty->display('sg_notas_ingresar_cm.tpl');
