...
$pdf->SetAutoPageBreak(true,10);  
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','B',16);
	$pdf->Cell(160,5,'CERTIFICADO DE ALUMNO REGULAR',0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	...
	$pdf->Row(array(utf8_decode('Josselyn Campos Sánchez, Directora del establecimiento educacional, '.$nombre_colegio.', de Villa Alemana, certifica que don (a) '.$nombre_alumno.', R.U.N.: '.$rut.', es alumno (a) de '.$nombre_curso.' y asiste regularmente a clases en el año escolar '.$anio.'.')),0);
	$pdf->Ln();
	$pdf->Ln();
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$alumno."_certificado_alumno_regular.pdf","F");
	$path = 'pdfs/'.$fecha.'_'.$alumno."_certificado_alumno_regular.pdf";  	
        ...
        $correo->From = 'direccion@elarcoiris.cl';
        $correo->FromName = 'Sistema Gestion Escuela Lenguaje Arcoiris';
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Certificado Alumno Regular - Escuela de Lenguaje Arcoiris');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = utf8_decode("Señor Apoderado,
						<br>
						De acuerdo a lo solicitado, se adjunta Certificado de Alumnos Regular.
						<br>
						Nota : Correo enviado automáticamente, favor no responder.
						<br>
						Atte.
						<br>
						Dirección
