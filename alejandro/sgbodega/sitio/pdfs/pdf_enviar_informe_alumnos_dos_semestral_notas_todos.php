...
$sql_ve = "select  distinct concat(`PaternoAlumno`,' ',`MaternoAlumno`,' , ',`NombresAlumno`) as nombre_alumno,
						...
						Matriculas.NroLista as NumeroLista, Pruebas.CoeficientePrueba, Ramos.Descripcion
				from alumnos".$anio."
					inner join Cursos
						on alumnos".$anio.".CodigoCurso = Cursos.CodigoCurso
				...
				group by nombre_alumno,	`NumeroRutAlumno`, Asignaturas.CodigoRamo, Cursos.CodigoCurso, NroLista
	...
	$pdf->Output('pdfs/'.$fecha.'_'.$curso."_notas_2_semestres_todos.pdf","F");
	$path = 'pdfs/'.$fecha.'_'.$curso.."_notas_2_semestres_todos.pdf";  	
		...
		$correo->isHTML(true);                   
        $correo->Subject = utf8_decode('Reporte Sistema Integrado de Gestion Escolar - INFORME DE RENDIMIENTO ESCOLAR');         

        $correo->AddAttachment($path, $asunto, $encoding = 'base64', $type = 'application/pdf');

        $correo->Body = "Envio de INFORME DE RENDIMIENTO ESCOLAR";
