...
$pdf->AddPage();
	...
	$nombre_asignatura_contador = 0;

	$rut_puntos = number_format($rut,0,',','.').'-'.dv($rut);
	...
	while ($line_ve = mysql_fetch_row($res_ve)) {
		...
		if ($line_ve[7]=='1'){
			...
			$promedio = $promedio > '0' ? round($promedio,1) : 0;
			$total_alumno += $promedio == 'NAN' ? 0 : round($promedio,1);
			if ($promedio>0){
				$cont_notas_alumno++;
				}

			$promedio = $contador_prom > 0 ? $total_prom/$contador_prom : $total_prom;				
			array_push($arrRegistrosDetalle, array("item"					=>	$j, 
														"rut_alumno"			=> 	$line_ve[1], 
														"nota"					=>	number_format($promedio,1,".","," ), 
														"CodigoRamo"			=>	$line_ve[2]));
	...
	$pdf->Row(array(utf8_decode('Alumno'),utf8_decode($nombre_alumno),utf8_decode('Rut'),$rut_puntos),0);
