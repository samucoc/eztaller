...
function validarSiNumero(numero,field, evt, salta){
    if (!/^[0-9]+(\.[0-9]{1,2})?$/.test(numero))
      return Tabula(field, evt, salta);
  }
	...
	for (i=0;i<document.Form1.elements.length;i++){
		if(document.Form1.elements[i].type != 'button'){
			...
			if(document.Form1.elements[i].type == 'select-one'){
				...
				if (((document.Form1.elements[i].value=='- - Seleccione - -') || (document.Form1.elements[i].value=='Elija') )&& (document.Form1.elements[i].name.substr(0,4) == 'OBLI')){
					document.Form1.elements[i].focus();
					document.Form1.elements[i].className='tabla-alycar-input-yellow';
					alert(mensaje);
					return 0;
