(function($, _) {
	...
	var init = function($mapWrapper){
		var maxZoom    = 16,
			zoom = parseInt( $mapWrapper.data('map-zoom') ) > 0 ? parseInt( $mapWrapper.data('map-zoom') ) : 4,
			...
			if ('undefined' !== locations && locations.length) {
				locations.forEach(function(location){
					...
					if ('undefined' !== location.icon && location.icon.length) {
							marker.setIcon(location.icon); //Check if Icon exists and add it to the Marker
					}
